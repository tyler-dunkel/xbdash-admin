...
var packageList = function packageList(projectContext) {
  var versions = [];
...
//   deploying to.
var recordPackages = function recordPackages(options) {
  ...
  Fiber(function () {
    ...
    try {
      ...
      var result = conn.call("recordAppPackages", appIdentifier, packages, details);
...
var logErrorIfInCheckout = function logErrorIfInCheckout(err) {
...
var getPackagesForAppIdInTest = function getPackagesForAppIdInTest(projectContext) {
  var conn = connectToPackagesStatsServer();
  var result;
  try {
...
var connectToPackagesStatsServer = function connectToPackagesStatsServer() {
  return new ServiceConnection(config.getPackageStatsServerUrl(), { _dontPrintErrors: true });
};
