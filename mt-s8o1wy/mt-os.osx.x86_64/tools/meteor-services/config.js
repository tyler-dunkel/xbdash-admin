...
_.extend(exports, {
  ...
  getPackageServerUrl: function getPackageServerUrl() {
    return process.env.METEOR_PACKAGE_SERVER_URL || "https://packages.meteor.com";
  },

  getPackageServerDomain: function getPackageServerDomain() {
    return url.parse(config.getPackageServerUrl()).host;
  },

  getPackageStatsServerUrl: function getPackageStatsServerUrl() {
    return process.env.METEOR_PACKAGE_STATS_SERVER_URL || "https://activity.meteor.com";
  },

  getPackageStatsServerDomain: function getPackageStatsServerDomain() {
  ...
  getPackageServerFilePrefix: function getPackageServerFilePrefix(serverUrl) {
    var self = this;
    if (!serverUrl) {
  ...
  getPackagesDirectoryName: function getPackagesDirectoryName(serverUrl) {
    var self = this;
  ...
  getLocalPackageCacheFilename: function getLocalPackageCacheFilename(serverUrl) {
    var self = this;
  ...
  getPackageStorage: function getPackageStorage(options) {
    var self = this;
    options = options || {};
    var root = options.root || tropohouse['default'].root;
    return files.pathJoin(root, "package-metadata", "v2.0.1", self.getLocalPackageCacheFilename(options.serverUrl));
  },

  getIsopacketRoot: function getIsopacketRoot() {
    if (files.inCheckout()) {
  ...
  // login token.
  getAccountsDomain: function getAccountsDomain() {
  ...
  // directory.
  getSessionFilePath: function getSessionFilePath() {
    // METEOR_SESSION_FILE is for automated testing purposes only.
    return process.env.METEOR_SESSION_FILE || files.pathJoin(files.getHomeDir(), '.meteorsession');
  }
});
