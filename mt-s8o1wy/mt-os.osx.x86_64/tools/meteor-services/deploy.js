...
var CAPABILITIES = ['showDeployMessages', 'canTransferAuthorization'];
...
var deployRpc = function deployRpc(options) {
  ...
  var deployURLBase = getDeployURL(options.site).await();
  ...
  var hasAllExpectedKeys = _.all(_.map(options.expectPayload || [], function (key) {
    return ret.payload && _.has(ret.payload, key);
  }));

  if (options.expectPayload && !_.has(ret, 'payload') || options.expectMessage && !_.has(ret, 'message') || !hasAllExpectedKeys) {
    delete ret.payload;
    delete ret.message;
...
//   RPC.
var authedRpc = function authedRpc(options) {
...
// claim'.
var printLegacyPasswordMessage = function printLegacyPasswordMessage(site) {
  Console.error("\nThis site was deployed with an old version of Meteor that used " + "site passwords instead of user accounts. Now we have a much better " + "system, Meteor developer accounts.");
  Console.error();
  Console.error("If this is your site, please claim it into your account with");
  Console.error(Console.command("meteor claim " + site), Console.options({ indent: 2 }));
};
...
var printUnauthorizedMessage = function printUnauthorizedMessage() {
...
var canonicalizeSite = function canonicalizeSite(site) {
...
var bundleAndDeploy = function bundleAndDeploy(options) {
  ...
  if (preflight.protection === "password") {
    ...
    return 1;
  } else if (preflight.protection === "account" && !preflight.authorized) {
...
var deleteApp = function deleteApp(site) {
...
// for this site).
var checkAuthThenSendRpc = function checkAuthThenSendRpc(site, operation, what) {
  var preflight = authedRpc({
    operation: operation,
    site: site,
    preflight: true,
...
// site's database.
var temporaryMongoUrl = function temporaryMongoUrl(site) {
  site = canonicalizeSite(site);
  if (!site) {
...
var listAuthorized = function listAuthorized(site) {
  site = canonicalizeSite(site);
  if (!site) {
    return 1;
  }
...
var changeAuthorized = function changeAuthorized(site, action, username) {
  var _qs;
  ...
  var result = authedRpc({
    ...
    site: site,
    qs: (_qs = {}, _qs[action] = username, _qs),
    promptIfAuthFails: true,
    printDeployURL: true
  });
...
var claim = function claim(site) {
  ...
  if (infoResult.statusCode === 404) {
    Console.error("There isn't a site deployed at that address. Use " + Console.command("'meteor deploy'") + " " + "if you'd like to deploy your app here.");
    return 1;
  }
  ...
  if (infoResult.payload && infoResult.payload.protection === "password") {
    Console.info("To claim this site and transfer it to your account, enter the", "site password one last time.");
    Console.info();
  }
  ...
  if (result.errorMessage) {
    auth.pollForRegistrationCompletion();
    if (!auth.loggedInUsername() && auth.registrationUrl()) {
      Console.error("You need to set a password on your Meteor developer account before", "you can claim sites. You can do that here in under a minute:");
  ...
  Console.info(Console.command("meteor authorized " + site), Console.options({ indent: 2 }));
  Console.info();
  Console.info("Add authorized users with:");
  Console.info(Console.command("meteor authorized " + site + " --add <username>"), Console.options({ indent: 2 }));
  Console.info();
  Console.info("Remove authorized users with:");
  Console.info(Console.command("meteor authorized " + site + " --remove <username>"), Console.options({ indent: 2 }));
  Console.info();
  return 0;
};

var listSites = function listSites() {
  var result = deployRpc({
    method: "GET",
...
var galaxyDiscoveryCache = new Map();
...
function getDeployURL(site) {
  ...
  var defaultURL = "https://us-east-1.galaxy-deploy.meteor.com";
  ...
  var p = discoverGalaxy(site, "https")['catch'](function () {
    return discoverGalaxy(site, "http");
  })['catch'](function () {
    return defaultURL;
  });
...
function discoverGalaxy(site, scheme) {
  var discoveryURL, _httpHelpers$request, response, body;

  return _regeneratorRuntime.async(function discoverGalaxy$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          discoveryURL = scheme + "://" + site + "/.well-known/meteor/deploy-url";
          // If httpHelpers.request throws, the returned Promise will reject, which is
          // fine.

          _httpHelpers$request = httpHelpers.request({
            url: discoveryURL,
            json: true,
            strictSSL: true,
          ...
          response = _httpHelpers$request.response;
          body = _httpHelpers$request.body;
        ...
        case 12:
          return _context.abrupt('return', body.deployURL);
