...
//
var ServiceConnection = function ServiceConnection(endpointUrl, options) {
  var self = this;
  ...
  self.connection._stream.on('disconnect', function (error) {
    ...
    if (self.currentPromise) {
      ...
      self.currentPromise = null;
      promise.reject(error || new Package['ddp-client'].DDP.ConnectionError("DDP disconnected while connection in progress"));
    } else if (error) {
...
_.extend(ServiceConnection.prototype, {
  ...
  apply: function apply() {
    ...
    (_self$connection = self.connection).apply.apply(_self$connection, args);
  ...
  subscribeAndWait: function subscribeAndWait() {
    var _self$connection2;
    ...
    var sub = (_self$connection2 = self.connection).subscribe.apply(_self$connection2, args);
    subPromise.await();
    return sub;
  },
