...
var getLoadedPackages = function getLoadedPackages() {
...
var openAccountsConnection = function openAccountsConnection() {
...
// `f` returns or throws.
var withAccountsConnection = function withAccountsConnection(f) {
  return function () {
    var self = this;
...
var loggedInAccountsConnection = function loggedInAccountsConnection(token) {
  var connection = getLoadedPackages()['ddp-client'].DDP.connect(config.getAuthDDPUrl());

  return new Promise(function (resolve, reject) {
    connection.apply('login', [{ resume: token }], { wait: true }, function (err) {
...
var sessionMethodCaller = function sessionMethodCaller(methodName, options) {
  options = options || {};
  return function () {
...
var readSessionData = function readSessionData() {
...
var writeSessionData = function writeSessionData(data) {
  var sessionPath = config.getSessionFilePath();
...
var getSession = function getSession(sessionData, domain) {
  if (_typeof(sessionData.sessions) !== "object") {
    sessionData.sessions = {};
  }
  if (_typeof(sessionData.sessions[domain]) !== "object") {
...
var ensureSessionType = function ensureSessionType(session, type) {
...
var writeMeteorAccountsUsername = function writeMeteorAccountsUsername(username) {
...
var logOutAllSessions = function logOutAllSessions(data) {
...
var logOutSession = function logOutSession(session) {
...
var currentUsername = function currentUsername(data) {
  var sessionData = getSession(data, config.getAccountsDomain());
  return sessionData.username || null;
};

var removePendingRevoke = function removePendingRevoke(domain, tokenIds) {
  var data = readSessionData();
...
var tryRevokeOldTokens = function tryRevokeOldTokens(options) {
  options = _.extend({
    timeout: 5000
  }, options || {});
  ...
  var logoutFailWarning = function logoutFailWarning(domain) {
    if (!warned) {
...
var sendAuthorizeRequest = function sendAuthorizeRequest(clientId, redirectUri, state) {
...
var oauthFlow = function oauthFlow(conn, options) {
  var crypto = require('crypto');
  var credentialToken = crypto.randomBytes(16).toString('hex');

  var authorizeResult = sendAuthorizeRequest(options.clientId, options.redirectUri, credentialToken);
...
var doInteractivePasswordLogin = function doInteractivePasswordLogin(options) {
  var loginData = {};
  ...
  var maybeCloseConnection = function maybeCloseConnection() {
...
exports.registerOrLogIn = withAccountsConnection(function (connection) {
  ...
  while (true) {
    ...
    // Try to register
    try {
      var methodCaller = sessionMethodCaller('tryRegister', { connection: connection });
...
exports.loginWithTokenOrOAuth = function (conn, accountsConfiguration, url, domain, sessionType) {
  var setUpOnReconnect = function setUpOnReconnect() {
