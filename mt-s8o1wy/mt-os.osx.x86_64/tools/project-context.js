var _toConsumableArray;module.import('babel-runtime/helpers/toConsumableArray',{"default":function(v){_toConsumableArray=v}});var KNOWN_ISOBUILD_FEATURE_PACKAGES;module.import('./isobuild/compiler.js',{"KNOWN_ISOBUILD_FEATURE_PACKAGES":function(v){KNOWN_ISOBUILD_FEATURE_PACKAGES=v}});
...
_.extend(ProjectContext.prototype, {
  reset: function reset(moreOptions, resetOptions) {
    var self = this;
    ...
    self._explicitlyAddedLocalPackageDirs = options.explicitlyAddedLocalPackageDirs;
    ...
    if (options.alwaysWritePackageMap && options.neverWritePackageMap) throw Error("always or never?");
    ...
    self._upgradeIndirectDepPatchVersions = options.upgradeIndirectDepPatchVersions;
  ...
  readProjectMetadata: function readProjectMetadata() {
  ...
  initializeCatalog: function initializeCatalog() {
    var _this = this;

    Profile.run('ProjectContext initializeCatalog', function () {
      _this._completeStagesThrough(STAGE.INITIALIZE_CATALOG);
    });
  },
  resolveConstraints: function resolveConstraints() {
    var _this2 = this;

    Profile.run('ProjectContext resolveConstraints', function () {
      _this2._completeStagesThrough(STAGE.RESOLVE_CONSTRAINTS);
    });
  },
  downloadMissingPackages: function downloadMissingPackages() {
    var _this3 = this;

    Profile.run('ProjectContext downloadMissingPackages', function () {
      _this3._completeStagesThrough(STAGE.DOWNLOAD_MISSING_PACKAGES);
    });
  },
  buildLocalPackages: function buildLocalPackages() {
    var _this4 = this;

    Profile.run('ProjectContext buildLocalPackages', function () {
      _this4._completeStagesThrough(STAGE.BUILD_LOCAL_PACKAGES);
    });
  },
  saveChangedMetadata: function saveChangedMetadata() {
    var _this5 = this;

    Profile.run('ProjectContext saveChangedMetadata', function () {
      _this5._completeStagesThrough(STAGE.SAVE_CHANGED_METADATA);
    });
  },
  prepareProjectForBuild: function prepareProjectForBuild() {
    var _this6 = this;

    // This is the same as saveChangedMetadata, but if we insert stages after
    // that one it will continue to mean "fully finished".
    Profile.run('ProjectContext prepareProjectForBuild', function () {
      _this6._completeStagesThrough(STAGE.SAVE_CHANGED_METADATA);
    });
  },

  _completeStagesThrough: function _completeStagesThrough(targetStage) {
    var self = this;
    ...
    buildmessage.enterJob('preparing project', function () {
      while (self._completedStage !== targetStage) {
        ...
        if (self.completedStage === STAGE.SAVE_CHANGED_METADATA) throw Error("can't find requested stage " + targetStage);
  ...
  getProjectLocalDirectory: function getProjectLocalDirectory(subdirectory) {
    var self = this;
    return files.pathJoin(self.projectLocalDir, subdirectory);
  },

  getMeteorShellDirectory: function getMeteorShellDirectory(projectDir) {
  ...
  writeReleaseFileAndDevBundleLink: function writeReleaseFileAndDevBundleLink(releaseName) {
    assert.strictEqual(files.inCheckout(), false);
    this.releaseFile.write(releaseName);
  },


  _ensureProjectDir: function _ensureProjectDir() {
    var self = this;
  ...
  getProjectWatchSet: function getProjectWatchSet() {
  ...
  // this one.
  getProjectAndLocalPackagesWatchSet: function getProjectAndLocalPackagesWatchSet() {
    var self = this;
  ...
  getLintingMessagesForLocalPackages: function getLintingMessagesForLocalPackages() {
    var self = this;
    ...
    catalog.runAndRetryWithRefreshIfHelpful(function (canRetry) {
      buildmessage.enterJob("selecting package versions", function () {
        ...
        try {
          ...
          if (!e.constraintSolverError && !e.versionParserError) throw e;
          ...
          buildmessage.error(e.message, { tags: { refreshCouldHelp: !!e.constraintSolverError } });
        }
  ...
  _readResolverResultCache: function _readResolverResultCache() {
  ...
  _saveResolverResultCache: function _saveResolverResultCache() {
  ...
  getOriginalAppDirForTestPackages: function getOriginalAppDirForTestPackages() {
    var appDir = this._projectDirForLocalPackages;
  ...
  _localPackageSearchDirs: function _localPackageSearchDirs() {
    var self = this;
    ...
    function packageDirsFromEnvVar(envVar) {
      var delimiter = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : files.pathOsDelimiter;

      return process.env[envVar] && process.env[envVar].split(delimiter) || [];
    }

    var envPackageDirs = [].concat(_toConsumableArray(packageDirsFromEnvVar('METEOR_PACKAGE_DIRS')), _toConsumableArray(packageDirsFromEnvVar('PACKAGE_DIRS', ':')));

    if (!self._ignorePackageDirsEnvVar && envPackageDirs.length) {
      // path.delimiter was added in v0.9.3
      envPackageDirs.forEach(function (p) {
        return searchDirs.push(files.pathResolve(p));
      });
    }
    ...
    if (!self._ignoreCheckoutPackages && files.inCheckout()) {
      ...
      searchDirs.push(packagesDir, files.pathJoin(packagesDir, "non-core", "*", "packages"));
    }
    return searchDirs;
  },
  ...
  _getRootDepsAndConstraints: function _getRootDepsAndConstraints() {
    var self = this;
  ...
  _addAppConstraints: function _addAppConstraints(depsAndConstraints) {
    var self = this;
  ...
  _addLocalPackageConstraints: function _addLocalPackageConstraints(depsAndConstraints) {
    var self = this;
    _.each(self.localCatalog.getAllPackageNames(), function (packageName) {
      ...
      var constraint = utils.parsePackageConstraint(packageName + "@=" + versionRecord.version);
  ...
  _getAnticipatedPrereleases: function _getAnticipatedPrereleases(rootConstraints, cachedVersions) {
    var self = this;

    var anticipatedPrereleases = {};
    var add = function add(packageName, version) {
      if (!/-/.test(version)) {
        return;
      }
  ...
  _buildResolver: function _buildResolver() {
    var self = this;

    var constraintSolverPackage = isopackets.load('constraint-solver')['constraint-solver'];
    var resolver = new constraintSolverPackage.ConstraintSolver.PackagesResolver(self.projectCatalog, {
  ...
  _saveChangedMetadata: Profile('_saveChangedMetadata', function () {
    ...
    if (!self._neverWriteProjectConstraintsFile) self.projectConstraintsFile.writeIfModified();
...
_.extend(exports.ProjectConstraintsFile.prototype, {
  _readFile: function _readFile() {
    ...
    _.each(lines, function (line) {
      var lineRecord = { leadingSpace: '', constraint: null, trailingSpaceAndComment: '' };
      ...
      line = match[1];
      lineRecord.trailingSpaceAndComment = match[2] + lineRecord.trailingSpaceAndComment;
      // Strip leading space.
      match = line.match(/^(\s*)((?:\S.*)?)$/);
  ...
  writeIfModified: function writeIfModified() {
    var self = this;
  ...
  _write: function _write() {
    ...
    var messages = buildmessage.capture({ title: 're-reading .meteor/packages' }, function () {
      self._readFile();
    });
  ...
  eachConstraint: function eachConstraint(iterator) {
    var self = this;
    _.each(self._constraintLines, function (lineRecord) {
      if (lineRecord.constraint) iterator(lineRecord.constraint);
    });
  },

  // Returns the constraint in the format returned by
  // utils.parsePackageConstraint, or null.
  getConstraint: function getConstraint(name) {
    var self = this;
  ...
  addConstraints: function addConstraints(constraintsToAdd) {
    var self = this;
  ...
  addPackages: function addPackages(packagesToAdd) {
  ...
  updateReleaseConstraints: function updateReleaseConstraints(releaseRecord) {
    var _this7 = this;

    this.addConstraints(_.compact(_.map(releaseRecord.packages, function (version, packageName) {
      if (_this7.getConstraint(packageName)) {
  ...
  // writeIfModified() which is called in the _saveChangedMetadata step
  // of project preparation.
  removePackages: function removePackages(packagesToRemove) {
    var self = this;
  ...
  removeAllPackages: function removeAllPackages() {
    var self = this;
    self._constraintLines = [];
    self._constraintMap = {};
    self._modified = true;
...
_.extend(exports.PackageMapFile.prototype, {
  ...
  getCachedVersions: function getCachedVersions() {
    var self = this;
    return _.clone(self._versions);
  },

  write: function write(packageMap) {
    var self = this;
...
_.extend(exports.PlatformList.prototype, {
  ...
  write: function write(platforms) {
    var self = this;
    self._platforms = null;
...
_.extend(exports.CordovaPluginsFile.prototype, {
  ...
  getPluginVersions: function getPluginVersions() {
    var self = this;
    return _.clone(self._plugins);
  },

  write: function write(plugins) {
    var self = this;
    ...
    files.writeFileAtomically(self.filename, lines.join(''));
    var messages = buildmessage.capture({ title: 're-reading .meteor/cordova-plugins' }, function () {
      self._readFile();
    });
    // We shouldn't choke on something we just wrote!
    if (messages.hasMessages()) throw Error("wrote bad .meteor/packages: " + messages.formatMessages());
...
_.extend(exports.ReleaseFile.prototype, {
  fileMissing: function fileMissing() {
    var self = this;
    return self.unnormalizedReleaseName === null;
  },
  noReleaseSpecified: function noReleaseSpecified() {
    var self = this;
  ...
  normalReleaseSpecified: function normalReleaseSpecified() {
    var self = this;
  ...
  _readFile: function _readFile() {
    ...
    var lines = _.compact(_.map(files.splitBufferToLines(contents), files.trimSpaceAndComments));
  ...
  getDevBundle: function getDevBundle() {
    var devBundle = files.getDevBundle();
    var devBundleParts = devBundle.split(files.pathSep);
    var meteorToolIndex = devBundleParts.lastIndexOf("meteor-tool");

    if (meteorToolIndex >= 0) {
      var releaseVersion = this.catalog.getReleaseVersion(this.releaseTrack, this.releaseVersion);

      if (releaseVersion) {
        var meteorToolVersion = releaseVersion.tool.split("@").pop();
  ...
  ensureDevBundleLink: function ensureDevBundleLink() {var makeLink,readLink;module.import("./cli/dev-bundle-links.js",{"makeLink":function(v){makeLink=v},"readLink":function(v){readLink=v}});
    

    var dotMeteorDir = files.pathDirname(this.filename);
    var localDir = files.pathJoin(dotMeteorDir, "local");
    var devBundleLink = files.pathJoin(localDir, "dev_bundle");
    ...
    var newTarget = this.getDevBundle();
    if (!newTarget) {
      return;
    }

    try {
      var oldOSPath = readLink(devBundleLink);
      var oldTarget = files.convertToStandardPath(oldOSPath);
  ...
  write: function write(releaseName) {
    var self = this;
...
_.extend(exports.FinishedUpgraders.prototype, {
  readUpgraders: function readUpgraders() {
    var self = this;
    var upgraders = [];
  ...
  appendUpgraders: function appendUpgraders(upgraders) {
    var self = this;
