...
_.extend(RunLog.prototype, {
  ...
  _clearSpecial: function _clearSpecial() {
    var self = this;
  ...
  setRawLogs: function setRawLogs(rawLogs) {
    this.rawLogs = !!rawLogs;
  },

  logAppOutput: function logAppOutput(line, isStderr) {
    var self = this;

    var Log = getLoggingPackage();

    var obj = isStderr ? Log.objFromText(line, { level: 'warn', stderr: true }) : Log.parse(line) || Log.objFromText(line);
    self._record(obj);
  ...
  //     calls in run-all.js
  logTemporary: function logTemporary(msg) {
    var self = this;
  ...
  logRestart: function logRestart() {
    var self = this;
  ...
  logClientRestart: function logClientRestart() {
    var self = this;
