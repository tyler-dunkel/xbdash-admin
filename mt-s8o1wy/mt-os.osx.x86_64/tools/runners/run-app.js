var _typeof;module.import('babel-runtime/helpers/typeof',{"default":function(v){_typeof=v}});var _regeneratorRuntime;module.import('babel-runtime/regenerator',{"default":function(v){_regeneratorRuntime=v}});var cordova={};module.import('../cordova',{"*":function(v,n){cordova[n]=v}});var CordovaBuilder;module.import('../cordova/builder.js',{"CordovaBuilder":function(v){CordovaBuilder=v}});var closeAllWatchers;module.import("../fs/safe-watcher.js",{"closeAllWatchers":function(v){closeAllWatchers=v}});
...
var bashParse = function bashParse(s) {
...
var getNodeOptionsFromEnvironment = function getNodeOptionsFromEnvironment() {
...
var AppProcess = function AppProcess(options) {
  var self = this;
...
_.extend(AppProcess.prototype, {
  ...
  start: function start() {
    ...
    function eachline(stream, encoding, callback) {
      realEachline(stream, encoding, function () {
        return void callback.apply(undefined, arguments);
      });
    }

    eachline(self.proc.stdout, 'utf8', function _callee(line) {
    ...
    eachline(self.proc.stderr, 'utf8', function _callee2(line) {
      return _regeneratorRuntime.async(function _callee2$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              if (!(self.debugPort && line.indexOf("debugger listening on port ") >= 0)) {
                _context2.next = 3;
                break;
              }
    ...
    // log lines).
    self.proc.on('close', function _callee3(code, signal) {
    ...
    self.proc.on('error', function _callee4(err) {
  ...
  _maybeCallOnExit: function _maybeCallOnExit(code, signal) {
    var self = this;
  ...
  _computeEnvironment: function _computeEnvironment() {
    var self = this;
  ...
  _spawn: function _spawn() {
    var self = this;

    // Path conversions
    var nodePath = process.execPath; // This path is an OS path already
    var entryPoint = files.convertToOSPath(files.pathJoin(self.bundlePath, 'main.js'));
...
var AppRunner = function AppRunner(options) {
  var self = this;
...
_.extend(AppRunner.prototype, {
  ...
  _makePromise: function _makePromise(name) {
    var self = this;
  ...
  _resolvePromise: function _resolvePromise(name, value) {
  ...
  _cleanUpPromises: function _cleanUpPromises() {
  ...
  makeBeforeStartPromise: function makeBeforeStartPromise() {
  ...
  _runOnce: function _runOnce(options) {
    var self = this;
    options = options || {};
    ...
    var bundleApp = function bundleApp() {
      if (!firstRun) {
      ...
      var bundleResult = Profile.run((firstRun ? "B" : "Reb") + "uild App", function () {
    ...
    var combinedWatchSetForBundleResult = function combinedWatchSetForBundleResult(br) {
    ...
    var cordovaRunner = self.cordovaRunner;
    if (cordovaRunner) {
      var _ret = function () {
        ...
        if (!cordovaRunner.started) {
          var _ret2 = function () {
            var settingsFile = self.settingsFile;
            var mobileServerUrl = self.mobileServerUrl;

            var messages = buildmessage.capture(function () {
              cordovaRunner.prepareProject(bundlePath, pluginVersions, { settingsFile: settingsFile, mobileServerUrl: mobileServerUrl });
            });
          ...
          if (cordovaRunner.havePlatformsChangedSinceLastRun()) {
            return {
              v: { outcome: 'outdated-cordova-platforms' }
            };
          }

          if (cordovaRunner.havePluginsChangedSinceLastRun(pluginVersions)) {
            return {
              v: { outcome: 'outdated-cordova-plugins' }
            };
          }
        }
      }();
    ...
    var appProcess = new AppProcess({
      ...
      onExit: function onExit(code, signal) {
    ...
    function maybePrintLintWarnings(bundleResult) {
      ...
      if (bundleResult.warnings.hasMessages()) {
        var formattedMessages = bundleResult.warnings.formatMessages();
        runLog.log('Linted your app.\n\n' + formattedMessages, { arrow: true });
      } else {
        runLog.log('Linted your app. No linting errors.', { arrow: true });
      }
    }
    ...
    var setupClientWatcher = function setupClientWatcher() {
  ...
  _fiber: function _fiber() {
    ...
    while (true) {
      ...
      var resetCrashCount = function resetCrashCount() {
      ...
      if (runResult.outcome === "wrong-release" || runResult.outcome === "conflicting-versions") {
        ...
        if (self.watchForChanges) {
          runLog.log("Your application has errors. " + "Waiting for file change.", { arrow: true });
          Console.enableProgressDisplay(false);
        }
        ...
        if (self.watchForChanges) {
          runLog.log("Your application is crashing. " + "Waiting for file change.", { arrow: true });
          Console.enableProgressDisplay(false);
        }
      } else {
