...
_.extend(Proxy.prototype, {
  ...
  _tryHandleConnections: function _tryHandleConnections() {
    var self = this;
    ...
    while (self.httpQueue.length) {
      ...
      if (self.mode === "errorpage") {
        ...
        attempt(c.res, function () {
          return self.proxy.web(c.req, c.res, {
    ...
    while (self.websocketQueue.length) {
      ...
      attempt(c.socket, function () {
        return self.proxy.ws(c.req, c.socket, c.head, {
          target: 'http://' + self.proxyToHost + ':' + self.proxyToPort
        });
      });
    }
...
function escapeEntities(str) {
  ...
  var escapeChar = function escapeChar(c) {
    return escapeMap[c];
  };
