...
// on that database.
var runMongoShell = function runMongoShell(url) {
  ...
  child_process.spawn(files.convertToOSPath(mongoPath), args, { stdio: 'inherit' });
};
...
function spawnMongod(mongodPath, port, dbPath, replSetName) {
  ...
  '--bind_ip', '127.0.0.1', '--port', port, '--dbpath', dbPath,
  ...
  if (process.platform === "win32" || process.platform === "linux" && process.arch === "ia32") {
...
if (process.platform === 'win32') {
  ...
  findMongoPids = function findMongoPids(dbDir_unused, port) {
    ...
    child_process.exec('tasklist /fi "IMAGENAME eq mongod.exe"', function (error, stdout, stderr) {
      if (error) {
        ...
        child_process.exec('netstat -ano', { maxBuffer: 1024 * 1024 * 10 }, function (error, stdout, stderr) {
          if (error) {
  ...
  findMongoPids = function findMongoPids(dbDir, port) {
    var promise = fiberHelpers.makeFulfillablePromise();
    ...
    { maxBuffer: 1024 * 1024 * 10 }, function (error, stdout, stderr) {
      if (error) {
...
var findMongoPort = function findMongoPort(dbDir) {
...
if (process.platform === 'win32') {
  ...
  findMongoPort = function findMongoPort(dbPath) {
    var mongoPort = null;
...
var findMongoAndKillItDead = function findMongoAndKillItDead(port, dbPath) {
...
var StoppedDuringLaunch = function StoppedDuringLaunch() {};
...
var launchMongo = function launchMongo(options) {
  ...
  if (process.env.METEOR_TEST_FAKE_MONGOD_CONTROL_PORT) {
    ...
    mongod_path = files.pathJoin(files.getCurrentToolsDir(), 'tools', 'tests', 'fake-mongod', fakeMongodCommand);
  ...
  var yieldingMethod = function yieldingMethod(object, methodName) {
    for (var _len = arguments.length, args = Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++) {
      args[_key - 2] = arguments[_key];
    }

    return Promise.race([stopPromise, new Promise(function (resolve, reject) {
      object[methodName].apply(object, args.concat([function (err, res) {
        err ? reject(err) : resolve(res);
      }]));
    })]).await();
  };

  var launchOneMongoAndWaitForReadyForInitiate = function launchOneMongoAndWaitForReadyForInitiate(dbPath, port, portFile) {
    ...
    var readyToTalkPromise = new Promise(function (resolve) {
      maybeReadyToTalk = function maybeReadyToTalk() {
    ...
    var stdoutOnData = fiberHelpers.bindEnvironment(function (data) {
      ...
      // lines
      if (/\[initandlisten\] Did not find local replica set configuration document at startup/.test(data) || /\[ReplicationExecutor\] Locally stored replica set configuration does not have a valid entry for the current node/.test(data)) {
  ...
  var initiateReplSetAndWaitForReady = function initiateReplSetAndWaitForReady() {
    try {
      // Load mongo so we'll be able to talk to it.
      var mongoNpmModule = isopackets.load('mongo')['npm-mongo'].NpmModuleMongodb;

      // Connect to the intended primary and start a replset.
      var db = new mongoNpmModule.Db('meteor', new mongoNpmModule.Server('127.0.0.1', options.port, { poolSize: 1 }), { safe: true });
      ...
      try {
        var config = yieldingMethod(db.admin(), "command", {
          replSetGetConfig: 1
        }).config;
...
var MongoRunner = function MongoRunner(options) {
  var self = this;
...
_.extend(MRp, {
  ...
  _startOrRestart: function _startOrRestart() {
    var self = this;
  ...
  _exited: function _exited(code, signal, stderr, detectedErrors) {
    var self = this;
  ...
  _allowStartupToReturn: function _allowStartupToReturn() {
    var self = this;
    if (self.resolveStartupPromise) {
  ...
  _mongoHosts: function _mongoHosts() {
    var self = this;
    var ports = [self.port];
    if (self.multiple) {
