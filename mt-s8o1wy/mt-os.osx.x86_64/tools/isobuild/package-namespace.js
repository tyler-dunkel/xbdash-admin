module.export({PackageNamespace:function(){return PackageNamespace}});var _classCallCheck;module.import("babel-runtime/helpers/classCallCheck",{"default":function(v){_classCallCheck=v}});var each,size,compact;module.import("underscore",{"each":function(v){each=v},"size":function(v){size=v},"compact":function(v){compact=v}});var inCheckout;module.import("../fs/files.js",{"inCheckout":function(v){inCheckout=v}});var buildmessage;module.import("../utils/buildmessage.js",{"default":function(v){buildmessage=v}});var packageVersionParser;module.import("../packaging/package-version-parser.js",{"default":function(v){packageVersionParser=v}});
...
var PackageNamespace = function () {
  function PackageNamespace(packageSource) {
    _classCallCheck(this, PackageNamespace);
  ...
  PackageNamespace.prototype.describe = function describe(options) {
    var source = this._packageSource;
    ...
    each(options, function (value, key) {
      if (key === "summary" || key === "git") {
        ...
        if (typeof value !== "string") {
          buildmessage.error("The package version (specified with " + "Package.describe) must be a string.");
        ...
        if (goodVersion && parsedVersion !== value) {
          buildmessage.error("The package version (specified with Package.describe) may not " + "contain a plus-separated build ID.");
          // Recover by pretending that the version was not set.
          goodVersion = false;
        }
        if (goodVersion) {
  ...
  PackageNamespace.prototype.onUse = function onUse(f) {
    if (!this._packageSource.isTest) {
      if (this._fileAndDepLoader) {
        buildmessage.error("duplicate onUse handler; a package may have " + "only one", { useMyCaller: true });
  ...
  PackageNamespace.prototype.on_use = function on_use(f) {
    this.onUse(f);
  };
  ...
  PackageNamespace.prototype.onTest = function onTest(f) {
    var isTest = this._packageSource.isTest;
    ...
    if (isTest) {
      if (this._fileAndDepLoader) {
        buildmessage.error("duplicate onTest handler; a package may have " + "only one", { useMyCaller: true });
        // Recover by ignoring the duplicate
        return;
      }
  ...
  PackageNamespace.prototype.on_test = function on_test(f) {
    this.onTest(f);
  };
  ...
  PackageNamespace.prototype.registerBuildPlugin = function registerBuildPlugin(options) {
    var isTest = this._packageSource.isTest;
    ...
    if (!('name' in options)) {
      buildmessage.error("build plugins require a name", { useMyCaller: true });
      // recover by ignoring plugin
      return;
    }

    var pluginInfo = this._packageSource.pluginInfo;

    if (options.name in pluginInfo) {
      buildmessage.error("this package already has a plugin named '" + options.name + "'", { useMyCaller: true });
      // recover by ignoring plugin
      return;
    }
  ...
  PackageNamespace.prototype._transitional_registerBuildPlugin = function _transitional_registerBuildPlugin(options) {
    this.registerBuildPlugin(options);
  };

  PackageNamespace.prototype.includeTool = function includeTool() {
    var source = this._packageSource;
    if (!inCheckout()) {
