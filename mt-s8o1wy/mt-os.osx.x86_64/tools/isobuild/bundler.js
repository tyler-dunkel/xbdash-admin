module.export({NodeModulesDirectory:function(){return NodeModulesDirectory}});var _possibleConstructorReturn;module.import('babel-runtime/helpers/possibleConstructorReturn',{"default":function(v){_possibleConstructorReturn=v}});var _inherits;module.import('babel-runtime/helpers/inherits',{"default":function(v){_inherits=v}});var _extends;module.import('babel-runtime/helpers/extends',{"default":function(v){_extends=v}});var _objectWithoutProperties;module.import('babel-runtime/helpers/objectWithoutProperties',{"default":function(v){_objectWithoutProperties=v}});var _toConsumableArray;module.import('babel-runtime/helpers/toConsumableArray',{"default":function(v){_toConsumableArray=v}});var _typeof;module.import('babel-runtime/helpers/typeof',{"default":function(v){_typeof=v}});var _classCallCheck;module.import('babel-runtime/helpers/classCallCheck',{"default":function(v){_classCallCheck=v}});var Builder;module.import('./builder.js',{"default":function(v){Builder=v}});var JsFile,CssFile;module.import('./minifier-plugin.js',{"JsFile":function(v){JsFile=v},"CssFile":function(v){CssFile=v}});var loadIsopacket;module.import('../tool-env/isopackets.js',{"load":function(v){loadIsopacket=v}});var CORDOVA_PLATFORM_VERSIONS;module.import('../cordova',{"CORDOVA_PLATFORM_VERSIONS":function(v){CORDOVA_PLATFORM_VERSIONS=v}});
...
var stripLeadingSlash = function stripLeadingSlash(p) {
  if (p.charAt(0) === '/') {
    return p.slice(1);
  }
...
// tests.
exports._mainJsContents = ["", "// The debugger pauses here when you run `meteor debug`, because this is ", "// the very first code to be executed by the server process. If you have ", "// not already added any `debugger` statements to your code, feel free to ", "// do so now, wait for the server to restart, then reload this page and ", '// click the |\u25B6 button to continue.', "process.argv.splice(2, 0, 'program.json');", "process.chdir(require('path').join(__dirname, 'programs', 'server'));", "require('./programs/server/boot.js');"].join("\n");
...
var NodeModulesDirectory = function () {
  function NodeModulesDirectory(_ref) {
    var packageName = _ref.packageName;
    var sourceRoot = _ref.sourceRoot;
    var sourcePath = _ref.sourcePath;
    var preferredBundlePath = _ref.preferredBundlePath;
    var _ref$local = _ref.local;
    var local = _ref$local === undefined ? false : _ref$local;
    var _ref$npmDiscards = _ref.npmDiscards;
    var npmDiscards = _ref$npmDiscards === undefined ? null : _ref$npmDiscards;

    _classCallCheck(this, NodeModulesDirectory);
    ...
    assert.strictEqual(typeof sourceRoot === 'undefined' ? 'undefined' : _typeof(sourceRoot), "string");
    this.sourceRoot = sourceRoot;

    // The absolute path (on local disk) to a directory that contains
    // the built node_modules to use.
    assert.strictEqual(typeof sourcePath === 'undefined' ? 'undefined' : _typeof(sourcePath), "string");
    ...
    this.isTestPackage = typeof packageName === "string" && /^local-test[:_]/.test(packageName);

    // Optionally, files to discard.
    this.npmDiscards = npmDiscards;
  }

  NodeModulesDirectory.prototype.copy = function copy() {
    return new this.constructor(this);
  };

  NodeModulesDirectory.prototype.isPortable = function isPortable() {
    return meteorNpm.dependenciesArePortable(this.sourcePath);
  };

  NodeModulesDirectory.prototype.rebuildIfNonPortable = function rebuildIfNonPortable() {
    return meteorNpm.rebuildIfNonPortable(this.sourcePath);
  };

  NodeModulesDirectory.prototype.getPreferredBundlePath = function getPreferredBundlePath(kind) {
    assert.ok(kind === "bundle" || kind === "isopack", kind);

    var relPath = files.pathRelative(this.sourceRoot, this.sourcePath);
    rejectBadPath(relPath);

    var isApp = !this.packageName;
    if (!isApp) {
      var relParts = relPath.split(files.pathSep);
      var name = colonConverter.convert(this.packageName.replace(/^local-test[:_]/, ""));
      ...
      relPath = files.pathJoin.apply(files, _toConsumableArray(relParts));
    }
  ...
  NodeModulesDirectory.prototype.toJSON = function toJSON() {
    return {
  ...
  NodeModulesDirectory.readDirsFromJSON = function readDirsFromJSON(node_modules, _ref2) {
    var _ref2$rebuildBinaries = _ref2.rebuildBinaries;
    var rebuildBinaries = _ref2$rebuildBinaries === undefined ? false : _ref2$rebuildBinaries;

    var callerInfo = _objectWithoutProperties(_ref2, ['rebuildBinaries']);

    assert.strictEqual(_typeof(callerInfo.sourceRoot), "string");

    var nodeModulesDirectories = Object.create(null);

    function add(moreInfo, path) {
      var info = _extends({}, callerInfo, moreInfo);
    ...
    if (rebuildBinaries) {
      _.each(nodeModulesDirectories, function (info, path) {
  ...
  NodeModulesDirectory.prototype.getProdPackagePredicate = function getProdPackagePredicate() {
    if (this._prodPackagePredicate) {
      return this._prodPackagePredicate;
    }

    var sourcePath = this.sourcePath;
    var prodPackageNames = meteorNpm.getProdPackageNames(sourcePath);
    if (!prodPackageNames) {
    ...
    var prodPackageTree = Object.create(null);
    var complete = Symbol();
    var maxPartCount = 0;

    Object.keys(prodPackageNames).forEach(function (name) {
      var parts = name.split("/");
      var tree = prodPackageTree;
    ...
    return this._prodPackagePredicate = function isWithinProdPackage(dir) {
      var parts = files.pathRelative(sourcePath, dir).split(files.pathSep);

      var start = parts.lastIndexOf("node_modules") + 1;
      ...
      for (var pos = start; pos < parts.length; ++pos) {
        var part = parts[pos];
        var branch = tree[part];
...
var File = function () {
  ...
  File.prototype.hash = function hash() {
    if (!this._hash) {
  ...
  File.prototype.contents = function contents(encoding) {
    if (!this._contents) {
      if (!this.sourcePath) {
  ...
  File.prototype.setContents = function setContents(b) {
  ...
  File.prototype.size = function size() {
  ...
  File.prototype.setUrlToHash = function setUrlToHash(fileAndUrlSuffix, urlSuffix) {
    urlSuffix = urlSuffix || "";
  ...
  File.prototype.addCacheBuster = function addCacheBuster() {
    if (!this.url) {
  ...
  File.prototype.setUrlFromRelPath = function setUrlFromRelPath(relPath) {
    var url = relPath;
  ...
  File.prototype.setTargetPathFromRelPath = function setTargetPathFromRelPath(relPath) {
    // XXX hack
  ...
  File.prototype.setSourceMap = function setSourceMap(sourceMap, root) {
    if (sourceMap === null || ['object', 'string'].indexOf(typeof sourceMap === 'undefined' ? 'undefined' : _typeof(sourceMap)) === -1) {
  ...
  File.prototype.setAssets = function setAssets(assets) {
    if (!_.isEmpty(assets)) {
      this.assets = assets;
    }
  };
...
var Target = function () {
  function Target(_ref3) {
    var packageMap = _ref3.packageMap;
    var isopackCache = _ref3.isopackCache;
    var sourceRoot = _ref3.sourceRoot;
    var arch = _ref3.arch;
    var cordovaPluginsFile = _ref3.cordovaPluginsFile;
    var buildMode = _ref3.buildMode;
    var bundlerCacheDir = _ref3.bundlerCacheDir;
  ...
  Target.prototype.make = function make(_ref4) {
    var _this = this;

    var packages = _ref4.packages;
    var minifyMode = _ref4.minifyMode;
    var addCacheBusters = _ref4.addCacheBusters;
    var minifiers = _ref4.minifiers;

    buildmessage.assertInCapture();

    buildmessage.enterJob("building for " + this.arch, function () {
      // Populate the list of unibuilds to load
      _this._determineLoadOrder({
        packages: packages || []
      });

      var sourceBatches = _this._runCompilerPlugins();
      ...
      // later!
      if (_this instanceof ClientTarget) {
        var minifiersByExt = {};
  ...
  Target.prototype._determineLoadOrder = function _determineLoadOrder(_ref5) {
    var _this2 = this;
    ...
    var isopackCache = this.isopackCache;
    ...
    buildmessage.enterJob('linking the program', function () {
      ...
      packages.forEach(function (p) {
        ...
        // modes.
        if (p.debugOnly && _this2.buildMode === 'production') {
          return;
        }
        if (p.prodOnly && _this2.buildMode !== 'production') {
          return;
        }
        if (p.testOnly && _this2.buildMode !== 'test') {
          return;
        }
      ...
      _this2.usedPackages = {}; // Map from package name to true;
      ...
      var add = function (unibuild) {
        ...
        var processUnibuild = function processUnibuild(usedUnibuild) {
          if (onStack[usedUnibuild.id]) {
            buildmessage.error("circular dependency between packages " + unibuild.pkg.name + " and " + usedUnibuild.pkg.name);
  ...
  Target.prototype._runCompilerPlugins = function _runCompilerPlugins() {
  ...
  Target.prototype._emitResources = function _emitResources(sourceBatches) {
    var _this3 = this;
    ...
    var jsOutputFilesMap = compilerPluginModule.PackageSourceBatch.computeJsOutputFilesMap(sourceBatches);
    ...
    sourceBatches.forEach(function (sourceBatch) {
      ...
      if (_this3.cordovaDependencies) {
        _.each(unibuild.pkg.cordovaDependencies, function (version, name) {
          _this3._addCordovaDependency(name, version,
          // use newer version if another version has already been added
          false);
        });
      }

      var name = unibuild.pkg.name || null;
      var isApp = !name;

      // Emit the resources
      var resources = sourceBatch.getResources(jsOutputFilesMap.get(name));
      ...
      resources.forEach(function (resource) {
        ...
        if (_.contains(['js', 'css'], resource.type)) {
          var _ret2 = function () {
            ...
            if (resource.type === 'js' && isOs) {
              ...
              _.each(unibuild.nodeModulesDirectories, function (nmd) {
                addNodeModulesDirToObject(nmd, _this3.nodeModulesDirectories);
        ...
        if (_.contains(['head', 'body'], resource.type)) {
          ...
          _this3[resource.type].push(resource.data);
          return;
        }
      ...
      _this3.watchSet.merge(unibuild.pkg.pluginWatchSet);
    });
  };
  ...
  Target.prototype.minifyJs = function minifyJs(minifierDef, minifyMode) {
    var sources = _.map(this.js, function (file) {
    ...
    buildmessage.enterJob('minifying app code', function () {
      try {
        ...
        markedMinifier(sources, { minifyMode: minifyMode });
      } catch (e) {
  ...
  Target.prototype.rewriteSourceMaps = function rewriteSourceMaps() {
    var rewriteSourceMap = function (sm) {
      sm.sources = sm.sources.map(function (path) {
        var prefix = 'meteor://\uD83D\uDCBBapp';
  ...
  Target.prototype._addCordovaDependency = function _addCordovaDependency(name, version, override) {
  ...
  Target.prototype._addDirectCordovaDependencies = function _addDirectCordovaDependencies() {
    var _this4 = this;
    ...
    _.each(this.cordovaPluginsFile.getPluginVersions(), function (version, name) {
      _this4._addCordovaDependency(name, version, true /* override any existing version */);
    });
  };
  ...
  Target.prototype._addCacheBusters = function _addCacheBusters(type) {
    this[type].forEach(function (file) {
      file.addCacheBuster();
    });
  };
  ...
  Target.prototype.getWatchSet = function getWatchSet() {
  ...
  Target.prototype.mostCompatibleArch = function mostCompatibleArch() {
    var _this5 = this;

    return archinfo.leastSpecificDescription(_.pluck(this.unibuilds, 'arch').filter(function (arch) {
      return archinfo.matches(_this5.arch, arch);
    }));
  };
...
['make', '_runCompilerPlugins', '_emitResources', 'minifyJs', 'rewriteSourceMaps'].forEach(function (method) {
...
var ClientTarget = function (_Target) {
  _inherits(ClientTarget, _Target);

  function ClientTarget(options) {
    _classCallCheck(this, ClientTarget);

    // CSS files. List of File. They will be loaded in the order given.
    var _this6 = _possibleConstructorReturn(this, _Target.call(this, options));
    ...
    if (!archinfo.matches(_this6.arch, 'web')) {
  ...
  ClientTarget.prototype.minifyCss = function minifyCss(minifierDef, minifyMode) {
    var _this7 = this;

    var sources = this.css.map(function (file) {
    ...
    this.css = _.flatten(sources.map(function (source) {
      return source._minifiedFiles.map(function (file) {
        var newFile = new File({
          info: 'minified css',
          data: new Buffer(file.data, 'utf8')
  ...
  ClientTarget.prototype.write = function write(builder, _ref6) {
    var _this9 = this;

    var minifyMode = _ref6.minifyMode;
    ...
    var eachResource = function (f) {
      ...
      ["js", "css", "asset"].forEach(function (type) {
        _this8[type].forEach(function (file) {
          f(file, type);
        });
      });
    }.bind(this);
    ...
    eachResource(function (file, type) {
      return builder.reserve(file.targetPath);
    });
    ...
    eachResource(function (file, type) {
      ...
      if (file.sourceMap) {
        ...
        manifestItem.sourceMap = builder.writeToGeneratedFilename(file.targetPath + '.map', { data: mapData });
    ...
    ['head', 'body'].forEach(function (type) {
      var data = _this9[type].join('\n');
      if (data) {
        var dataBuffer = new Buffer(data, 'utf8');
        var dataFile = builder.writeToGeneratedFilename(type + '.html', { data: dataBuffer });
        manifest.push({
          path: dataFile,
          where: 'internal',
          type: type,
    ...
    if (this.arch === 'web.cordova') {
      (function () {
        ...
        var cordovaCompatibilityVersions = _.object(_.map(CORDOVA_PLATFORM_VERSIONS, function (version, platform) {
          var hash = WebAppHashing.calculateCordovaCompatibilityHash(version, _this9.cordovaDependencies);
...
['minifyCss', 'write'].forEach(function (method) {
...
var JsImage = function () {
  ...
  JsImage.prototype.load = function load(bindings) {
    var self = this;
    var ret = {};
    ...
    var getAsset = function getAsset(assets, assetPath, encoding, callback) {
    ...
    var nodeModulesDirsByPackageName = new Map();

    _.each(self.jsToLoad, function (item) {
      _.each(item.nodeModulesDirectories, function (nmd) {
        if (nmd.local) {
      ...
      var env = _.extend({
        ...
        Npm: {
          require: Profile(function (name) {
            ...
            _.some(item.nodeModulesDirectories, function (nmd) {
              if (nmd.local) {
              ...
              var nodeModulesTopDir = files.pathJoin(nmd.sourcePath, name.split("/")[0]);

              if (files.exists(nodeModulesTopDir)) {
                return fullPath = files.convertToOSPath(files.pathJoin(nmd.sourcePath, name));
              }
            });
            ...
            try {
              ...
              buildmessage.error("Can't load npm module '" + name + "' from " + item.targetPath + ". Check your Npm.depends().");
              return undefined;
            }
          })
        },
  ...
  JsImage.prototype._makeNpmRequire = function _makeNpmRequire(nodeModulesDirsByPackageName) {
    ...
    function npmResolve(id) {
      ...
      if (parts[start] === "node_modules" && parts[start + 1] === "meteor" && parts[start + 3] === "node_modules") {
        var packageName = colonConverter.convert(parts[start + 2]);
        dirs = nodeModulesDirsByPackageName.get(packageName);
        start += 4;
      } else {
        dirs = [];

        var appDirs = nodeModulesDirsByPackageName.get(null);
        if (appDirs) {
          var _dirs;

          (_dirs = dirs).push.apply(_dirs, _toConsumableArray(appDirs));
        }
        ...
        var modulesDirs = nodeModulesDirsByPackageName.get("modules");
        if (modulesDirs) {
          var _dirs2;

          (_dirs2 = dirs).push.apply(_dirs2, _toConsumableArray(modulesDirs));
        }
      ...
      if (dirs && dirs.length > 0) {
        var _ret4 = function () {
          var relativePath = parts.slice(start).join("/");
          var fullPath = void 0;

          _.some(dirs, function (dir) {
            var osPath = files.convertToOSPath(files.pathJoin(dir, relativePath));
  ...
  JsImage.prototype.write = function write(builder) {
    var _ref7 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

    var buildMode = _ref7.buildMode;
    var includeNodeModules = _ref7.includeNodeModules;
    ...
    _.each(self.jsToLoad, function (item) {
      ...
      _.each(item.nodeModulesDirectories, function (nmd) {
        ...
        var generatedNMD = nodeModulesDirectories[nmd.sourcePath];
        if (generatedNMD) {
          assert.strictEqual(_typeof(generatedNMD.preferredBundlePath), "string");

          loadItem.node_modules[generatedNMD.preferredBundlePath] = generatedNMD.toJSON();
        }
      });

      var preferredPaths = Object.keys(loadItem.node_modules);
      ...
      if (item.sourceMap) {
        ...
        loadItem.sourceMap = builder.writeToGeneratedFilename(sourceMapBaseName, { data: new Buffer(JSON.stringify(item.sourceMap), 'utf8') });
      ...
      if (!_.isEmpty(item.assets)) {
        ...
        _.each(item.assets, function (data, relPath) {
          ...
          if (_.has(assetFilesBySha, sha)) {
            ...
            loadItem.assets[relPath] = assetFilesBySha[sha] = builder.writeToGeneratedFilename(files.pathJoin(assetBundlePath, relPath), { data: data });
          }
        });
      }
    ...
    var rebuildDirs = Object.create(null);
    ...
    _.each(nodeModulesDirectories, function (nmd) {
      assert.strictEqual(_typeof(nmd.preferredBundlePath), "string");

      if (!nmd.isPortable()) {
        var parentDir = files.pathDirname(nmd.preferredBundlePath);
    ...
    builder.write("npm-rebuilds.json", {
      data: new Buffer(JSON.stringify(Object.keys(rebuildDirs), null, 2) + "\n", "utf8")
    });
  ...
  JsImage.readFromDisk = function readFromDisk(controlFilePath) {
    var ret = new JsImage();
    ...
    _.each(json.load, function (item) {
      ...
      var loadItem = {
        ...
        nodeModulesDirectories: nodeModulesDirectories
      };
...
['load', 'write'].forEach(function (method) {
  JsImage.prototype[method] = Profile('JsImage#' + method, JsImage.prototype[method]);
});

var JsImageTarget = function (_Target2) {
  _inherits(JsImageTarget, _Target2);

  function JsImageTarget(options) {
    _classCallCheck(this, JsImageTarget);

    var _this10 = _possibleConstructorReturn(this, _Target2.call(this, options));

    if (!archinfo.matches(_this10.arch, "os")) {
  ...
  JsImageTarget.prototype.toJsImage = function toJsImage() {
    var self = this;
    var ret = new JsImage();
  ...
  return JsImageTarget;
}(Target);

//////////////////// ServerTarget ////////////////////

var ServerTarget = function (_JsImageTarget) {
  _inherits(ServerTarget, _JsImageTarget);
  ...
  function ServerTarget(options) {
    _classCallCheck(this, ServerTarget);

    for (var _len = arguments.length, args = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      args[_key - 1] = arguments[_key];
    }

    var _this11 = _possibleConstructorReturn(this, _JsImageTarget.call.apply(_JsImageTarget, [this, options].concat(args)));

    _this11.clientTargets = options.clientTargets;
    _this11.releaseName = options.releaseName;
    _this11.appIdentifier = options.appIdentifier;

    if (!archinfo.matches(_this11.arch, "os")) {
  ...
  ServerTarget.prototype.write = function write(builder, _ref8) {
    var buildMode = _ref8.buildMode;
    var includeNodeModules = _ref8.includeNodeModules;
    var getRelativeTargetPath = _ref8.getRelativeTargetPath;
    ...
    // boot.js.
    var serverPkgJson = JSON.parse(files.readFile(files.pathJoin(files.getDevBundle(), 'etc', 'package.json')));
    ...
    builder.write('package.json', {
      data: new Buffer(JSON.stringify(serverPkgJson, null, 2) + "\n", "utf8")
    });
    ...
    var toolsDir = files.pathDirname(files.convertToStandardPath(__dirname));
    ...
    // Server bootstrap
    _.each(["boot.js", "boot-utils.js", "server-json.js", "mini-files.js", "npm-require.js", "npm-rebuild.js", "npm-rebuild-args.js"], function (filename) {
...
var writeTargetToPath = Profile("bundler writeTargetToPath", function (name, target, outputPath, _ref9) {
  var includeNodeModules = _ref9.includeNodeModules;
  var getRelativeTargetPath = _ref9.getRelativeTargetPath;
  var previousBuilder = _ref9.previousBuilder;
  var buildMode = _ref9.buildMode;
  var minifyMode = _ref9.minifyMode;
...
var writeSiteArchive = Profile("bundler writeSiteArchive", function (targets, outputPath, _ref10) {
  var includeNodeModules = _ref10.includeNodeModules;
  var builtBy = _ref10.builtBy;
  var releaseName = _ref10.releaseName;
  var getRelativeTargetPath = _ref10.getRelativeTargetPath;
  var previousBuilders = _ref10.previousBuilders;
  var buildMode = _ref10.buildMode;
  var minifyMode = _ref10.minifyMode;


  var builders = {};
  var previousStarBuilder = previousBuilders && previousBuilders.star;
  ...
  try {
    ...
    var _ret5 = function () {
      ...
      dependencySources.forEach(function (s) {
      ...
      Object.keys(targets).forEach(function (name) {
        ...
        var previousBuilder = previousBuilders && previousBuilders[name] ? previousBuilders[name] : null;

        var _writeTargetToPath = writeTargetToPath(name, target, builder.buildPath, {
          includeNodeModules: includeNodeModules,
          builtBy: builtBy,
        ...
        var arch = _writeTargetToPath.arch;
        var path = _writeTargetToPath.path;
        var cordovaDependencies = _writeTargetToPath.cordovaDependencies;
        var targetNP = _writeTargetToPath.nodePath;
        var targetBuilder = _writeTargetToPath.builder;


        builders[name] = targetBuilder;

        json.programs.push({
          name: name, arch: arch, path: path, cordovaDependencies: cordovaDependencies
        });
...
function bundle(_ref11) {
  var projectContext = _ref11.projectContext;
  var outputPath = _ref11.outputPath;
  var includeNodeModules = _ref11.includeNodeModules;
  var buildOptions = _ref11.buildOptions;
  var previousBuilders = _ref11.previousBuilders;
  var hasCachedBundle = _ref11.hasCachedBundle;
  ...
  var minifyMode = buildOptions.minifyMode || 'development';
  var buildMode = buildOptions.buildMode || 'production';
  ...
  var bundlerCacheDir = projectContext.getProjectLocalDirectory('bundler-cache');
  ...
  var messages = buildmessage.capture({
    ...
    var makeClientTarget = Profile("bundler.bundle..makeClientTarget", function (app, webArch, options) {
      var client = new ClientTarget({
        bundlerCacheDir: bundlerCacheDir,
    ...
    var mergeAppWatchSets = function mergeAppWatchSets() {
    ...
    _.each(webArchs, function (arch) {
      var client = makeClientTarget(app, arch, { minifiers: minifiers });
    ...
    var getRelativeTargetPath = function getRelativeTargetPath(options) {
      var pathForTarget = function pathForTarget(target) {
        var name;
    ...
    if (outputPath !== null) {
      if (hasCachedBundle) {
        ...
        _.each(targets, function (target, name) {
          var previousBuilder = previousBuilders && previousBuilders[name];
          var targetBuild = writeTargetToPath(name, target, outputPath, _.extend({
            buildMode: buildOptions.buildMode
          }, writeOptions, { previousBuilder: previousBuilder }));
        ...
        starResult = writeSiteArchive(targets, outputPath, _.extend({
          buildMode: buildOptions.buildMode
        }, writeOptions, { previousBuilders: previousBuilders }));
...
function lintBundle(projectContext, isopack, packageSource) {
  ...
  var lintedAnything = false;
  var lintingMessages = new buildmessage._MessageSet();

  if (projectContext.lintAppAndLocalPackages) {
    var _compiler$lint = compiler.lint(packageSource, {
      isopack: isopack,
      isopackCache: projectContext.isopackCache
    });

    var appMessages = _compiler$lint.warnings;
    var linted = _compiler$lint.linted;
  ...
  var localPackagesMessages = projectContext.getLintingMessagesForLocalPackages();
