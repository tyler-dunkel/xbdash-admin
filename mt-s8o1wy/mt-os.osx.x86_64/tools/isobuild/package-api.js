module.export({PackageAPI:function(){return PackageAPI}});var pathRelative,convertToPosixPath;module.import("../fs/files.js",{"pathRelative":function(v){pathRelative=v},"convertToPosixPath":function(v){convertToPosixPath=v}});var assert = require("assert");
...
function toArchArray(arch) {
  ...
  for (var i = 0; i < arch.length; ++i) {
    ...
    if (!isMatch) {
      buildmessage.error("Invalid 'where' argument: '" + inputArch + "'",
...
_.extend(PackageAPI.prototype, {
   ...
   */
  use: function use(names, arch, options) {
    var self = this;
    ...
    if (options.unordered && options.weak) {
      buildmessage.error("A dependency may not be both unordered and weak.", { useMyCaller: true });
      // recover by ignoring
      return;
    }
   ...
   * 'os.linux']`.
   */
  imply: function imply(names, arch) {
    var self = this;
    ...
    for (var i = 0; i < names.length; ++i) {
      ...
      if (compiler.isIsobuildFeaturePackage(parsed["package"])) {
        buildmessage.error("to declare that your package requires the build tool feature " + "'{parsed.package}', use 'api.use', not 'api.imply'");
        // recover by ignoring
        continue;
      }
   ...
   */
  addFiles: function addFiles(paths, arch, fileOptions) {
  ...
  mainModule: function mainModule(path, arch) {
    var _this = this;

    arch = toArchArray(arch);
    forAllMatchingArchs(arch, function (a) {
      var filesForArch = _this.files[a];
      var source = {
   ...
   */
  addAssets: function addAssets(paths, arch) {
    if (!arch) {
      buildmessage.error('addAssets requires a second argument specifying ' + 'where the asset should be available. For example: "client", ' + '"server", or ["client", "server"].', { useMyCaller: true });
      return;
    }
   ...
   */
  _addFiles: function _addFiles(type, paths, arch, fileOptions) {
    ...
    _.each(paths, function (path) {
      forAllMatchingArchs(arch, function (a) {
        var filesOfType = self.files[a][type];
   ...
   */
  versionsFrom: function versionsFrom(releases) {
    var self = this;
    ...
    if (self.buildingIsopackets) {
      buildmessage.error("packages in isopackets may not use versionsFrom");
      // recover by ignoring
      return;
    }
    ...
    for (var i = 0; i < releases.length; ++i) {
      ...
      if (relInf.length !== 2) {
        buildmessage.error("Release names in versionsFrom may not contain '@'.", { useMyCaller: true });
        return;
      }
      var releaseRecord = catalog.official.getReleaseVersion(relInf[0], relInf[1]);
      if (!releaseRecord) {
        buildmessage.error("Unknown release " + release, { tags: { refreshCouldHelp: true } });
      } else {
   ...
   */
  "export": function _export(symbols, arch, options) {
    var self = this;
    ...
    _.each(symbols, function (symbol) {
      ...
      if (!symbol.match(/^([_$a-zA-Z][_$a-zA-Z0-9]*)$/)) {
        buildmessage.error("Bad exported symbol: " + symbol, { useMyCaller: true });
        // recover by ignoring
        return;
      }
