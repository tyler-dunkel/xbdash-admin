module.export({IsopackCache:function(){return IsopackCache}});var _classCallCheck;module.import('babel-runtime/helpers/classCallCheck',{"default":function(v){_classCallCheck=v}});
...
var IsopackCache = function () {
  function IsopackCache(options) {
    _classCallCheck(this, IsopackCache);
  ...
  IsopackCache.prototype.buildLocalPackages = function buildLocalPackages(rootPackageNames) {
    var self = this;
  ...
  IsopackCache.prototype.wipeCachedPackages = function wipeCachedPackages(packages) {
    var self = this;
    if (packages) {
  ...
  IsopackCache.prototype.getIsopack = function getIsopack(name) {
    var self = this;
  ...
  IsopackCache.prototype.eachBuiltIsopack = function eachBuiltIsopack(iterator) {
    var self = this;
  ...
  IsopackCache.prototype.getSourceRoot = function getSourceRoot(name, arch) {
    var packageInfo = this._packageMap.getInfo(name);
    ...
    if (packageInfo) {
      ...
      if (packageInfo.kind === "versioned") {
        var isopackPath = this._tropohouse.packagePath(name, packageInfo.version);
  ...
  IsopackCache.prototype.uses = function uses(isopack, name, arch) {
    var _this = this;
    ...
    return _.some(isopack.unibuilds, function (u) {
      if (arch && !archinfo.matches(u.arch, arch)) {
        return false;
      }

      return _.some(u.uses, function (use) {
        return _this.implies(_this._isopacks[use['package']], name, arch);
      });
    });
  };

  IsopackCache.prototype.implies = function implies(isopack, name, arch) {
    var _this2 = this;
    ...
    return _.some(isopack.unibuilds, function (u) {
      ...
      return _.some(u.implies, function (imp) {
        return _this2.implies(_this2._isopacks[imp['package']], name, arch);
      });
    });
  };

  IsopackCache.prototype._ensurePackageLoaded = function _ensurePackageLoaded(name, onStack) {
    var self = this;
    buildmessage.assertInCapture();
    if (_.has(self._isopacks, name)) {
      return;
    ...
    var ensureLoaded = function ensureLoaded(depName) {
      if (_.has(onStack, depName)) {
        buildmessage.error("circular dependency between packages " + name + " and " + depName);
    ...
    if (packageInfo.kind === 'local') {
      ...
      buildmessage.enterJob("preparing to build package " + name, function () {
        ...
        Profile.time('IsopackCache Build local isopack', function () {
      ...
      Profile.time('IsopackCache Load local isopack', function () {
        if (previousIsopack) {
  ...
  IsopackCache.prototype._loadLocalPackage = function _loadLocalPackage(name, packageInfo, previousIsopack) {
    var self = this;
    buildmessage.assertInCapture();
    buildmessage.enterJob("building package " + name, function () {
      ...
      if (previousIsopack && self._checkUpToDatePreloaded(previousIsopack)) {
        ...
        if (upToDate) {
          ...
          // about.)
          isopack.setPluginProviderPackageMap(self._packageMap.makeSubsetMap(_.keys(isopackBuildInfoJson.pluginProviderPackageMap)));
  ...
  IsopackCache.prototype._lintLocalPackage = function _lintLocalPackage(packageSource, isopack) {
    ...
    var warnings = _compiler$lint.warnings;
  ...
  IsopackCache.prototype._checkUpToDate = function _checkUpToDate(isopackBuildInfoJson) {
    var self = this;
  ...
  IsopackCache.prototype._checkUpToDatePreloaded = function _checkUpToDatePreloaded(previousIsopack) {
    var self = this;
  ...
  IsopackCache.prototype._isopackDir = function _isopackDir(packageName) {
    var self = this;
    return files.pathJoin(self.cacheDir, colonConverter.convert(packageName));
  };

  IsopackCache.prototype._pluginCacheDirForPackage = function _pluginCacheDirForPackage(packageName) {
    var self = this;
    return files.pathJoin(self._pluginCacheDirRoot, colonConverter.convert(packageName));
  };

  IsopackCache.prototype._pluginCacheDirForVersion = function _pluginCacheDirForVersion(packageName, version) {
    var self = this;
    return files.pathJoin(self._pluginCacheDirForPackage(packageName), version);
  };

  IsopackCache.prototype._pluginCacheDirForLocal = function _pluginCacheDirForLocal(packageName) {
    var self = this;
    // assumes that `local` is not a valid package version.
    return files.pathJoin(self._pluginCacheDirForPackage(packageName), 'local');
  };

  IsopackCache.prototype._isopackBuildInfoPath = function _isopackBuildInfoPath(packageName) {
    var self = this;
    return files.pathJoin(self._isopackDir(packageName), 'isopack-buildinfo.json');
  };

  IsopackCache.prototype.forgetPreviousIsopackCache = function forgetPreviousIsopackCache() {
    var self = this;
    self._previousIsopackCache = null;
  };

  IsopackCache.prototype._shouldLintPackage = function _shouldLintPackage(packageSource) {
  ...
  IsopackCache.prototype.getLintingMessagesForLocalPackages = function getLintingMessagesForLocalPackages() {
    var _this3 = this;

    var messages = new buildmessage._MessageSet();
    var anyLinters = false;

    this._packageMap.eachPackage(function (name, packageInfo) {
      var isopack = _this3._isopacks[name];
      if (packageInfo.kind === 'local') {
        if (!_this3._shouldLintPackage(packageInfo.packageSource)) {
          return;
        }
