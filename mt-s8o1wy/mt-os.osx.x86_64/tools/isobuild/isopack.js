var Builder;module.import('./builder.js',{"default":function(v){Builder=v}});var assert = require('assert');
...
var rejectBadPath = function rejectBadPath(p) {
  if (p.match(/\.\./)) {
...
var nextBuildId = 1;
var Unibuild = function Unibuild(isopack, options) {
  var self = this;
  options = options || {};
  self.pkg = isopack;
  ...
  // debugging easier.
  self.id = self.pkg.name + "." + self.kind + "@" + self.arch + "#" + nextBuildId++;
  ...
  _.some(self.nodeModulesDirectories, function (nmd, nodeModulesPath) {
    if (!nmd.local) {
...
Isopack.readMetadataFromDirectory = Profile("Isopack.readMetadataFromDirectory", function (isopackDirectory) {
  ...
  return { metadata: metadata, originalVersion: originalVersion };
});
...
_.extend(Isopack.prototype, {
  ...
  initFromOptions: function initFromOptions(options) {
    var self = this;
  ...
  // constructor.
  addUnibuild: function addUnibuild(options) {
    var self = this;
    self.unibuilds.push(new Unibuild(self, options));
  },

  setPluginProviderPackageMap: function setPluginProviderPackageMap(pluginProviderPackageMap) {
    var self = this;
  ...
  getSourceFilesUnderSourceRoot: Profile("Isopack#getSourceFilesUnderSourceRoot", function (sourceRoot) {
    ...
    var addSourceFilesFromWatchSet = function addSourceFilesFromWatchSet(watchSet) {
  ...
  // package.
  buildArchitectures: function buildArchitectures() {
    var self = this;
  ...
  // binary builds)
  platformSpecific: function platformSpecific() {
    var self = this;
  ...
  tarballName: function tarballName() {
    var self = this;
    return colonConverter.convert(self.name) + '-' + self.version;
  },

  _toolArchitectures: function _toolArchitectures() {
    var self = this;
  ...
  getUnibuildAtArch: Profile("Isopack#getUnibuildAtArch", function (arch) {
    ...
    var chosenArch = archinfo.mostSpecificMatch(arch, _.pluck(self.unibuilds, 'arch'));
    ...
    if (!chosenArch) {
      buildmessage.error((self.name || "this app") + " is not compatible with architecture '" + arch + "'", { secondary: true });
  ...
  _checkPluginsInitialized: function _checkPluginsInitialized() {
    var self = this;
  ...
  ensurePluginsInitialized: Profile("Isopack#ensurePluginsInitialized", function () {
    ...
    self.sourceProcessors.compiler = new buildPluginModule.SourceProcessorSet(self.displayName(), { hardcodeJs: true, singlePackage: true });
    self.sourceProcessors.linter = new buildPluginModule.SourceProcessorSet(self.displayName(), { singlePackage: true, allowConflicts: true });
    self.sourceProcessors.minifier = new buildPluginModule.SourceProcessorSet(self.displayName(), { singlePackage: true });

    _.each(self.plugins, function (pluginsByArch, name) {
      var arch = archinfo.mostSpecificMatch(archinfo.host(), _.keys(pluginsByArch));
      if (!arch) {
        buildmessage.error("package `" + name + "` is built for incompatible " + "architecture");
      ...
      buildmessage.enterJob({
        title: "loading plugin `" + name + "` from package `" + self.name + "`"
    ...
    _.each(self.sourceProcessors, function (sourceProcessorSet) {
      _.each(sourceProcessorSet.allSourceProcessors, function (sourceProcessor) {
  ...
  _makePluginApi: function _makePluginApi() {
    var isopack = this;
    ...
    var Plugin = {
       ...
       * XXX COMPAT WITH 1.1
       */
      registerSourceHandler: function registerSourceHandler(extension, options, handler) {
        if (!handler) {
          handler = options;
          options = {};
        }
      ...
      _registerSourceProcessor: function _registerSourceProcessor(_ref, factory, _ref2) {
        var extensions = _ref.extensions;
        var filenames = _ref.filenames;
        var archMatching = _ref.archMatching;
        var isTemplate = _ref.isTemplate;
        var sourceProcessorSet = _ref2.sourceProcessorSet;
        var methodName = _ref2.methodName;
        var featurePackage = _ref2.featurePackage;
        ...
        var hasExtensions = extensions && extensions instanceof Array && extensions.length > 0;
        var hasFilenames = filenames && filenames instanceof Array && filenames.length > 0;
        if (!(hasExtensions || hasFilenames)) {
          buildmessage.error("Plugin." + methodName + " must specify a " + "non-empty array of extensions or filenames", { useMyCaller: 3 });
          // recover by ignoring
          return;
        }
        ...
        if (extensions && extensions.some(function (e) {
          return e.endsWith('/');
        })) {
          buildmessage.error('Plugin.' + methodName + ': extensions may not end in /');
          // recover by ignoring
          return;
        }
        if (filenames && filenames.some(function (f) {
          return f.endsWith('/');
        })) {
          buildmessage.error('Plugin.' + methodName + ': filenames may not end in /');
          // recover by ignoring
          return;
        }

        if (typeof factory !== 'function') {
          buildmessage.error(methodName + " call must " + "specify a factory function", { useMyCaller: 3 });
          // recover by ignoring
          return;
        }

        var sp = new buildPluginModule.SourceProcessor({
          isopack: isopack,
       ...
       * @locus Build Plugin
       */
      registerCompiler: function registerCompiler(options, factory) {
       ...
       * @locus Build Plugin
       */
      registerLinter: function registerLinter(options, factory) {
        Plugin._registerSourceProcessor(options || {}, factory, {
       ...
       * @locus Build Plugin
       */
      registerMinifier: function registerMinifier(options, factory) {
  ...
  _loadUnibuildsFromPath: function _loadUnibuildsFromPath(name, dir, options) {
    var self = this;
    options = options || {};
    ...
    var _Isopack$readMetadata = Isopack.readMetadataFromDirectory(dir);

    var mainJson = _Isopack$readMetadata.metadata;
    ...
    _.each(mainJson.builds, function (unibuildMeta) {
      ...
      unibuildJson.uses && unibuildJson.uses.forEach(function (use) {
      ...
      var rebuildBinaries = archinfo.matches(archinfo.host(), unibuildMeta.arch);

      var nodeModulesDirectories = bundler.NodeModulesDirectory.readDirsFromJSON(unibuildJson.node_modules, {
  ...
  hasCordovaUnibuild: function hasCordovaUnibuild() {
    var self = this;
  ...
  canWriteLegacyBuilds: function canWriteLegacyBuilds() {
    ...
    return this.unibuilds.every(function (unibuild) {
      return unibuild.resources.every(isResourceSafeForLegacyBuilds);
    });
  },
  ...
  saveToPath: Profile("Isopack#saveToPath", function (outputDir) {
    ...
    var includePreCompilerPluginIsopackVersions = _ref3.includePreCompilerPluginIsopackVersions;
    var includeIsopackBuildInfo = _ref3.includeIsopackBuildInfo;
    var _ref3$isopackCache = _ref3.isopackCache;
    var isopackCache = _ref3$isopackCache === undefined ? null : _ref3$isopackCache;
    ...
    try {
      ...
      (function () {
        ...
        var writeLegacyBuilds = includePreCompilerPluginIsopackVersions && self.canWriteLegacyBuilds();

        isopackBuildInfoJson = null;
        ...
        var npmDirsToCopy = Object.create(null);
        ...
        builder.write("package.js", {
          data: new Buffer("// This file is included for compatibility with the Meteor " + "0.6.4 package downloader.\n", "utf8")
        });
        ...
        _.each(self.unibuilds, function (unibuild) {
          ...
          var unibuildDir = builder.generateFilename(baseUnibuildName, { directory: true });
          ...
          _.each(unibuild.nodeModulesDirectories, function (nmd) {
            var bundlePath = _.has(npmDirsToCopy, nmd.sourcePath)
            // We already have this npm directory from another unibuild.
            ? npmDirsToCopy[nmd.sourcePath] : npmDirsToCopy[nmd.sourcePath] = builder.generateFilename(nmd.getPreferredBundlePath("isopack"), { directory: true });
            node_modules[bundlePath] = nmd.toJSON();
          });

          var preferredPaths = Object.keys(node_modules);
          ...
          var usesModules = !isopackCache || isopackCache.uses(self, "modules", unibuild.arch);
          ...
          _.each(unibuild.resources, function (resource) {
            ...
            var generatedFilename = builder.writeToGeneratedFilename(files.pathJoin(unibuildDir, resource.servePath || resource.path), { data: resource.data });
        ...
        _.each(npmDirsToCopy, function (bundlePath, sourcePath) {
        ...
        _.each(self.plugins, function (pluginsByArch, name) {
          _.each(pluginsByArch, function (plugin) {
            ...
            var pluginDir = builder.generateFilename('plugin.' + colonConverter.convert(name) + '.' + plugin.arch, { directory: true });
        ...
        if (self.includeTool) {
          toolsJson = self._writeTool(builder);
        ...
        if (writeLegacyBuilds) {
          ...
          _.each(unibuildInfos, function (unibuildInfo) {
            ...
            if (prelinkFile && prelinkData) {
              var prelinkResource = {
                type: 'prelink',
                file: builder.writeToGeneratedFilename(files.pathJoin(legacyDir, prelinkFile.servePath), { data: prelinkData }),
              ...
              if (prelinkFile.sourceMap) {
                ...
                prelinkResource.sourceMap = builder.writeToGeneratedFilename(files.pathJoin(legacyDir, prelinkFile.servePath + '.map'), { data: new Buffer(prelinkFile.sourceMap, 'utf8') });
              }
          ...
          // XXX COMPAT WITH 0.9.3
          builder.writeJson("unipackage.json", Isopack.convertIsopackFormat(
  ...
  _writeTool: Profile("Isopack#_writeTool", function (builder) {
    ...
    var pathsToCopy = utils.runGitInCheckout('ls-tree', '-r', '--name-only', '--full-tree', 'HEAD',
    // The actual trees to copy!
    'tools', 'examples', 'LICENSE.txt', 'LICENSES', 'meteor', 'meteor.bat', 'scripts/admin/launch-meteor', 'packages/package-version-parser/package-version-parser.js', 'packages/meteor/flush-buffers-on-exit-in-windows.js');
    ...
    /^tools\/packaging\/[^\/]+\.js$/, /^tools\/packaging\/catalog\/[^\/]+\.js$/, /^tools\/utils\/[^\/]+\.js$/, /^tools\/fs\/[^\/]+\.js$/, /^tools\/meteor-services\/[^\/]+\.js$/, /^tools\/tool-testing\/[^\/]+\.js$/, /^tools\/console\/[^\/]+\.js$/, /^tools\/cordova\/[^\/]+\.js$/];
    ...
    pathsToCopy.forEach(function (path) {
      var shouldTranspile = _.some(transpileRegexes, function (regex) {
    ...
    pathsToTranspile.forEach(function (path) {
  ...
  featureEnabled: function featureEnabled(featurePackageName) {
    return this.isobuildFeatures.indexOf(featurePackageName) !== -1;
  },


  _getServePath: function _getServePath(pathInPackage) {
    var self = this;
    var serveRoot;
    if (self.name) {
    ...
    return colonConverter.convert(files.pathJoin(serveRoot,
