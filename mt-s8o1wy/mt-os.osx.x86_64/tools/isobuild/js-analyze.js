module.export({findImportedModuleIdentifiers:function(){return findImportedModuleIdentifiers},findAssignedGlobals:function(){return findAssignedGlobals}});var _typeof;module.import('babel-runtime/helpers/typeof',{"default":function(v){_typeof=v}});var parse;module.import('meteor-babel',{"parse":function(v){parse=v}});var analyzeScope;module.import('escope',{"analyze":function(v){analyzeScope=v}});var LRU;module.import("lru-cache",{"default":function(v){LRU=v}});
...
function findImportedModuleIdentifiers(source, hash) {
  ...
  var ast = tryToParse(source, hash);
  ...
  function walk(node, left, right, requireIsBound) {
    if (left >= right) {
      ...
      while (left < right && possibleIndexes[left] < start) {
        ++left;
      }while (left < right && end < possibleIndexes[right - 1]) {
        --right;
      }if (left < right) {
        ...
        var id = getRequiredModuleId(node);
        if (typeof id === "string") {
        ...
        for (var _iterator = Object.keys(node), _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
          var _ref;
  ...
  function addIdentifier(id, type, requireIsBound) {
    var entry = hasOwn.call(identifiers, id) ? identifiers[id] : identifiers[id] = { possiblySpurious: true };
...
function isNode(value) {
  return value && (typeof value === 'undefined' ? 'undefined' : _typeof(value)) === "object" && typeof value.type === "string" && typeof value.start === "number" && typeof value.end === "number";
}
...
function isFunctionWithParameter(node, name) {
  if (node.type === "FunctionExpression" || node.type === "FunctionDeclaration" || node.type === "ArrowFunctionExpression") {
    return node.params.some(function (param) {
      return isIdWithName(param, name);
    });
  }
}
...
function getImportedModuleId(node) {
  if (node.type === "CallExpression" && node.callee.type === "MemberExpression" && isIdWithName(node.callee.object, "module") && isIdWithName(node.callee.property, "import")) {
    var args = node.arguments;
    var argc = args.length;
    if (argc > 0) {
      var arg = args[0];
...
var globalsCache = new LRU({
  max: Math.pow(2, 12),
  length: function length(globals) {
    var sum = 0;
...
function findAssignedGlobals(source, hash) {
  ...
  // ignore.
  var scopeManager = analyzeScope(ast, {
    ecmaVersion: 6,
    sourceType: "module",
    ignoreEval: true,
  ...
  var program = ast.type === "File" ? ast.program : ast;
  var programScope = scopeManager.acquire(program);
  var assignedGlobals = {};
  ...
  programScope.implicit.variables.forEach(function (variable) {
  ...
  programScope.implicit.left.forEach(function (entry) {
    if (entry.identifier && entry.identifier.type === "Identifier" &&
