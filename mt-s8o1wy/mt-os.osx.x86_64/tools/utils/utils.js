...
exports.ipAddress = function () {
  ...
  if (defaultRoute) {
    ...
    var addressEntry = _.findWhere(interfaces[iface], { family: "IPv4" });
...
exports.printPackageList = function (items, options) {
  ...
  var alphaSort = function alphaSort(row) {
    return row[0];
  };
...
exports.defaultOrderKeyForReleaseVersion = function (v) {
  ...
  var hasRedundantLeadingZero = function hasRedundantLeadingZero(x) {
    return x.length > 1 && x[0] === '0';
  };
  var leftPad = function leftPad(chr, len, str) {
    if (str.length > len) {
  ...
  var rightPad = function rightPad(chr, len, str) {
    if (str.length > len) {
      throw Error("too long to pad!");
    }
    var padding = new Array(len - str.length + 1).join(chr);
...
exports.generateSubsetsOfIncreasingSize = function (total, cb) {
  ...
  var generateSubsetsOfFixedSize = function generateSubsetsOfFixedSize(goalSize, chosenIndices) {
...
exports.ensureOnlyValidVersions = function (dependencies, _ref) {
  var forCordova = _ref.forCordova;
