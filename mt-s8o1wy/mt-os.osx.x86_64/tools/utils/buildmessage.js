...
_.extend(Job.prototype, {
  ...
  hasMessageWithTag: function hasMessageWithTag(tagName) {
    var self = this;
  ...
  formatMessages: function formatMessages(indent) {
    var self = this;
    var out = "";
    var already = {};
...
// set of messages.
var MessageSet = function MessageSet(messageSet) {
  var self = this;
  self.jobs = [];
...
_.extend(MessageSet.prototype, {
  ...
  // MessageSet.
  merge: function merge(messageSet) {
    var self = this;
...
var getRootProgress = function getRootProgress() {
  return rootProgress;
};

var reportProgress = function reportProgress(state) {
...
var reportProgressDone = function reportProgressDone() {
  var progress = currentProgress.get();
  if (progress) {
...
var getCurrentProgressTracker = function getCurrentProgressTracker() {
  var progress = currentProgress.get();
  return progress ? progress : rootProgress;
};

var addChildTracker = function addChildTracker(title) {
  var options = {};
  if (title !== undefined) {
    options.title = title;
  }
...
// `options`.
var capture = function capture(options, f) {
  var messageSet = new MessageSet();
  var parentMessageSet = currentMessageSet.get();

  var title;
  if ((typeof options === 'undefined' ? 'undefined' : _typeof(options)) === "object" && options.title) {
    title = options.title;
  }
...
var enterJob = function enterJob(options, f) {
...
var jobHasMessages = function jobHasMessages() {
  var search = function search(job) {
    if (job.hasMessages()) {
      return true;
    }
...
// code that called it).
var markBoundary = function markBoundary(f) {
...
//   tag.
var error = function error(message, options) {
  options = options || {};
  ...
  if ('useMyCaller' in info) {
    if (info.useMyCaller) {
      var _parseStack$parse = parseStack.parse(new Error());

      var insideFiber = _parseStack$parse.insideFiber;
      var outsideFiber = _parseStack$parse.outsideFiber;
...
var assertInJob = function assertInJob() {
  if (!currentJob.get()) {
    throw new Error("Expected to be in a buildmessage job");
  }
};

var assertInCapture = function assertInCapture() {
  if (!currentMessageSet.get()) {
    throw new Error("Expected to be in a buildmessage capture");
  }
};

var mergeMessagesIntoCurrentJob = function mergeMessagesIntoCurrentJob(innerMessages) {
...
var forkJoin = function forkJoin(options, iterable, fn) {
  if (!_.isFunction(fn)) {
    fn = iterable;
    iterable = options;
    options = {};
  ...
  return enterJobAsync(options).then(function () {
    var errors = [];
