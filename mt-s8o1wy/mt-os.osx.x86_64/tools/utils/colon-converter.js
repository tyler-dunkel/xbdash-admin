...
//   ]
// }
var convertBySchema = function convertBySchema(val, schema) {
  if (schema === true) {
    return convert(val);
  ...
  if (_.isArray(schema)) {
    ...
    if (!_.isArray(val)) {
      throw new Error("Expected an array in value, got " + (typeof val === "undefined" ? "undefined" : _typeof(val)));
    }
  ...
  if (!_.isObject(schema)) {
    throw new Error("Unexpected type of schema: " + (typeof schema === "undefined" ? "undefined" : _typeof(schema)));
  }
...
var convert = function convert(path) {
