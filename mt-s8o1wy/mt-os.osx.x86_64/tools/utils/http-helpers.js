...
var ConcatStream = function (_Writable) {
  _inherits(ConcatStream, _Writable);

  function ConcatStream() {
    _classCallCheck(this, ConcatStream);

    var _this = _possibleConstructorReturn(this, _Writable.call(this));
  ...
  ConcatStream.prototype._write = function _write(chunk, encoding, next) {
    this.chunks.push(chunk);
    this.size += chunk.length;
    next();
  };

  ConcatStream.prototype.getBuffer = function getBuffer() {
  ...
  ConcatStream.prototype.end = function end() {
    ...
    if (force === true) {
      _Writable.prototype.end.call(this);
    }
  };
...
var WritableWithProgress = function WritableWithProgress(writable, listener) {
  var self = this;
...
_.extend(WritableWithProgress.prototype, {
  ...
  _progress: function _progress(n, done) {
    var self = this;
  ...
  once: function once() {
    ...
    return (_inner = this._inner).once.apply(_inner, arguments);
  },

  emit: function emit() {
    var _inner2;

    return (_inner2 = this._inner).emit.apply(_inner2, arguments);
  }
});
...
_.extend(exports, {
  ...
  request: function request(urlOrOptions, callback) {
    var options;
    ...
    if (!callback) {
      promise = new Promise(function (resolve, reject) {
        callback = function callback(err, response, body) {
          if (err) {
            reject(err);
            return;
          }
    ...
    var req = request(options, function (error, response, body) {
      if (!error && response && body) {
        ...
        if (contentLength > 0 && body.length < contentLength) {
          error = new Error("Expected " + contentLength + " bytes in request body " + "but received only " + body.length);
        }
      }
  ...
  _addProgressEvents: function _addProgressEvents(request) {
    var state = {};

    var emitProgress = function emitProgress() {
  ...
  getUrl: function getUrl(urlOrOptions) {
    try {
  ...
  // other scenarioes.
  getUrlWithResuming: function getUrlWithResuming(urlOrOptions) {
    var options = _.isObject(urlOrOptions) ? _.clone(urlOrOptions) : {
      url: urlOrOptions
    };

    var maxAttempts = _.has(options, "maxAttempts") ? options.maxAttempts : 10;

    var retryDelaySecs = _.has(options, "retryDelaySecs") ? options.retryDelaySecs : 5;

    var masterProgress = options.progress;
    var outputStream = new ConcatStream();

    function attempt() {
      var triesRemaining = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : maxAttempts;
      ...
      try {
        ...
        var _ret = function () {
          ...
          var useTry = size === startAt;
          var change = size - startAt;
          ...
          if (!useTry || triesRemaining > 0) {
            ...
            return {
              v: new Promise(function (resolve) {
                return setTimeout(resolve, retryDelaySecs * 1000);
              }).then(function () {
                return attempt(triesRemaining - (useTry ? 1 : 0), size);
              })
            };
          }
          ...
          return {
            v: Promise.reject(new files.OfflineError(e))
          };
        }();
