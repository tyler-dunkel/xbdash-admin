...
function start(debugPort, entryPoint) {
  ...
  debugEntries[debugPort] = entry = new DebugEntry(debugPort, entryPoint);
...
DEp.startInspector = function startInspector() {
  ...
  var proc = spawn(process.execPath, [require.resolve("node-inspector/bin/inspector"), "--web-port", "" + webPort, "--debug-port", "" + debugPort]);
...
DEp.connectToChildProcess = function connectToChildProcess(child) {
  ...
  child.stderr.on("data", function onData(buffer) {
    var match = /debugger listening on port (\d+)/i.exec(buffer.toString("utf8"));
    if (match) {
  ...
  function connect(port) {
    ...
    protocol.onResponse = function onResponse(res) {
      ...
      if (res.body.type === "event" && res.body.event === "break") {
        var scriptName = res.body.body.script.name;
        var lineNumber = res.body.body.sourceLine + 1;

        if (self.connectCount > 1 && scriptName === self.entryPoint) {
          ...
          process.stdout.write("Paused at " + scriptName + ":" + lineNumber + "\n");
        }
      }
    };
...
function banner(debugPort) {
  ...
  assert.strictEqual(_typeof(proc.url), "string");

  return ["", chalk.green(["Your application is now ready for debugging!", "", "To debug the server process using a graphical debugging interface, ", "visit this URL in your web browser:", ""].join(EOL)), chalk.cyan("  " + proc.url), chalk.green(["", "If your application is paused on a breakpoint but the code is not ", "visible in the debugger, press the pause (||) button.", ""].join(EOL)), ""].join(EOL);
}
