...
var CARRIAGE_RETURN = process.platform === 'win32' && process.stdout.isTTY ? new Array(249).join('\b') : '\r';
...
var spacesString = function spacesString(length) {
...
var toFixedLength = function toFixedLength(text, length) {
  text = text || "";
...
// display' case
var ProgressDisplayNone = function ProgressDisplayNone() {};

_.extend(ProgressDisplayNone.prototype, {
  depaint: function depaint() {
    // No-op
  },
...
var ProgressDisplayStatus = function ProgressDisplayStatus(console) {
  var self = this;
...
var SpinnerRenderer = function SpinnerRenderer() {
  var self = this;
  self.frames = ['-', '\\', '|', '/'];
  self.start = +new Date();
  self.interval = 250;
...
var ProgressBarRenderer = function ProgressBarRenderer(format, options) {
  var self = this;
...
_.extend(ProgressBarRenderer.prototype, {
  asString: function asString(availableSpace) {
    var self = this;
    ...
    var str = self.fmt.replace(':current', self.curr).replace(':total', self.total).replace(':elapsed', isNaN(elapsed) ? '0.0' : (elapsed / 1000).toFixed(1)).replace(':eta', isNaN(eta) || !isFinite(eta) ? '0.0' : (eta / 1000).toFixed(1)).replace(':percent', percent.toFixed(0) + '%');
...
var ProgressDisplayFull = function ProgressDisplayFull(console) {
  var self = this;
...
_.extend(ProgressDisplayFull.prototype, {
  ...
  updateProgress: function updateProgress(fraction, startTime) {
    var self = this;
  ...
  setHeadless: function setHeadless(headless) {
  ...
  _render: function _render() {
    ...
    if (self._status || progressGraphic) {
      ...
      if (self._headless && line === self._lastWrittenLine) {
...
var StatusPoller = function StatusPoller(console) {
  var self = this;
...
_.extend(StatusPoller.prototype, {
  _startPoller: function _startPoller() {
    var _this = this;
    ...
    self._pollPromise = function _callee() {
  ...
  _statusPoll: function _statusPoll() {
    var self = this;
    ...
    var reportState = function reportState(state, startTime) {
...
var ConsoleOptions = function ConsoleOptions(o) {
  var self = this;
  self.options = o;
};
...
_.extend(Console.prototype, {
  ...
  setPretty: function setPretty(pretty) {
    var self = this;
  ...
  withProgressDisplayVisible: function withProgressDisplayVisible(f) {
    var self = this;
  ...
  setVerbose: function setVerbose(verbose) {
    var self = this;
  ...
  //
  _parseVariadicInput: function _parseVariadicInput(args) {
    var self = this;
    var msgArgs;
    var options;
  ...
  isLevelEnabled: function isLevelEnabled(levelCode) {
  ...
  clear: function clear() {
    ...
    self.rawInfo('\x1B[2J\x1B[0;0H');
  },

  _prettifyMessage: function _prettifyMessage(msgArguments) {
    var self = this;
  ...
  _print: function _print(level, message) {
    var self = this;
  ...
  success: function success(message) {
    var uglySuccessKeyword = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "success";
  ...
  _fail: function _fail(message, printFn) {
    ...
    var xmark = chalk.red('\u2717');
    return self[printFn](chalk.red(message), self.options({ bulletPoint: xmark + " " }));
  },

  // Wrapper around Console.warn that prints a large "WARNING" label in front.
  labelWarn: function labelWarn(message) {
    var self = this;
  ...
  arrowError: function arrowError(message, indent) {
    var self = this;
    return self._arrowPrint("error", message, indent);
  },
  arrowWarn: function arrowWarn(message, indent) {
    var self = this;
    return self._arrowPrint("warn", message, indent);
  },
  arrowInfo: function arrowInfo(message, indent) {
    var self = this;
    return self._arrowPrint("info", message, indent);
  },
  _arrowPrint: function _arrowPrint(printFn, message, indent) {
    var self = this;
    indent = indent || 0;
    return self[printFn](message, self.options({ bulletPoint: ARROW, indent: indent }));
  },
  ...
  // level, etc.
  printError: function printError(err, info) {
    var self = this;
  ...
  printMessages: function printMessages(messages) {
    var self = this;
  ...
  url: function url(message) {
    ...
    var unspaced = replaceAll(message, ' ', '%20');
  ...
  printTwoColumns: function printTwoColumns(rows, options) {
    var self = this;
    options = options || {};
  ...
  _format: function _format(logArguments) {
  ...
  //
  _wrapText: function _wrapText(text, options) {
    var self = this;
    options = options || {};
    ...
    if (process.env.METEOR_NO_WORDWRAP) {
      ...
      if (options.bulletPoint) {
        ...
        var initIndent = options.indent ? wrappedText.substring(0, options.indent) : "";
  ...
  enableProgressDisplay: function enableProgressDisplay(enabled) {
    var self = this;
  ...
  _updateProgressDisplay: function _updateProgressDisplay() {
    var self = this;
  ...
  setHeadless: function setHeadless() {
    var headless = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
  ...
  _setProgressDisplay: function _setProgressDisplay(newProgressDisplay) {
    var self = this;
