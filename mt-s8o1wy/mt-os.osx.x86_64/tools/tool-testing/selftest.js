...
var _require = require("../utils/http-helpers.js");

var getUrlWithResuming = _require.getUrlWithResuming;
...
function checkTestOnlyDependency(name) {
  try {
    ...
    throw new Error(["Please install " + name + " by running the following command:", "", "  /path/to/meteor npm install -g " + name, "", "Where `/path/to/meteor` is the executable you used to run this self-test.", ""].join("\n"));
  }
...
var TestFailure = function TestFailure(reason, details) {
  var self = this;
  self.reason = reason;
...
// to the user.
var markStack = function markStack(f) {
...
var doOrThrow = function doOrThrow(f) {
  var ret;
...
var setUpBuiltPackageTropohouse = function setUpBuiltPackageTropohouse() {
  ...
  var versions = {};
  _.each(tropohouseLocalCatalog.getAllNonTestPackageNames(), function (packageName) {
...
var newSelfTestCatalog = function newSelfTestCatalog() {
  if (!files.inCheckout()) {
  ...
  var messages = buildmessage.capture({ title: "scanning local core packages" }, function () {
...
var Matcher = function Matcher(run) {
  var self = this;
  self.buf = "";
  self.ended = false;
...
_.extend(Matcher.prototype, {
  ...
  match: function match(pattern, timeout, strict) {
    var self = this;
  ...
  matchEmpty: function matchEmpty() {
    var self = this;
...
var OutputLog = function OutputLog(run) {
  var self = this;
...
_.extend(OutputLog.prototype, {
  write: function write(channel, text) {
    var self = this;
  ...
  forbid: function forbid(pattern, channel) {
    var self = this;
    _.each(self.lines, function (line) {
      ...
      var match = pattern instanceof RegExp ? line.text.match(pattern) : line.text.indexOf(pattern) !== -1;
      if (match) {
...
var Sandbox = function Sandbox(options) {
  var self = this;
  // default options
  ...
  if (options.clients && options.clients.browserstack) {
    var browsers = [{ browserName: 'firefox' }, { browserName: 'chrome' }, { browserName: 'internet explorer',
      browserVersion: '11' }, { browserName: 'internet explorer',
      browserVersion: '8',
      timeout: 60 }, { browserName: 'safari' }, { browserName: 'android' }];
...
_.extend(Sandbox.prototype, {
  ...
  // });
  testWithAllClients: function testWithAllClients(f) {
  ...
  //   s.cd('myapp');
  createApp: function createApp(to, template, options) {
    var self = this;
    options = options || {};
    var absoluteTo = files.pathJoin(self.cwd, to);
    files.cp_r(files.pathJoin(files.convertToStandardPath(__dirname), '..', 'tests', 'apps', template), absoluteTo, { ignore: [/^local$/] });
    ...
    var upgradersFile = new projectContextModule.FinishedUpgraders({ projectDir: absoluteTo });
  ...
  //   s.cd('me_mypack');
  createPackage: function createPackage(packageDir, packageName, template) {
    var self = this;
    var packagePath = files.pathJoin(self.cwd, packageDir);
    var templatePackagePath = files.pathJoin(files.convertToStandardPath(__dirname), '..', 'tests', 'packages', template);
  ...
  //     s.run('add', 'somepackage');
  //   });
  cd: function cd(relativePath, callback) {
    var self = this;
  ...
  unlink: function unlink(filename) {
    var self = this;
  ...
  rename: function rename(oldName, newName) {
    var self = this;
    files.rename(files.pathJoin(self.cwd, oldName), files.pathJoin(self.cwd, newName));
  },

  // Return the current contents of .meteorsession in the sandbox.
  readSessionFile: function readSessionFile() {
    var self = this;
  ...
  writeSessionFile: function writeSessionFile(contents) {
    var self = this;
    return files.writeFile(files.pathJoin(self.root, '.meteorsession'), contents, 'utf8');
  },

  _makeEnv: function _makeEnv() {
    var self = this;
    var env = _.clone(self.env);
    env.METEOR_SESSION_FILE = files.convertToOSPath(files.pathJoin(self.root, '.meteorsession'));
  ...
  _makeWarehouse: function _makeWarehouse(releases) {
    var self = this;
    ...
    // And a cherry on top
    // XXX this is hacky
    files.linkToMeteorScript(files.pathJoin(self.warehouse, packagesDirectoryName, "meteor-tool", toolPackageVersion, 'mt-' + archinfo.host(), 'meteor'), files.pathJoin(self.warehouse, 'meteor'));
  }
});
...
var Client = function Client(options) {
  ...
  self.url = "http://" + self.host + ":" + self.port + '/' + (Math.random() * 0x100000000 + 1).toString(36);
...
// PhantomClient
var PhantomClient = function PhantomClient(options) {
  var self = this;
...
_.extend(PhantomClient.prototype, {
  connect: function connect() {
    ...
    var scriptPath = files.pathJoin(files.getCurrentToolsDir(), "tools", "tool-testing", "phantom", "open-url.js");
    self.process = child_process.execFile(phantomPath, ["--load-images=no", files.convertToOSPath(scriptPath), self.url], {}, function (error, stdout, stderr) {
...
var BrowserStackClient = function BrowserStackClient(options) {
  var self = this;
  Client.apply(this, arguments);
...
_.extend(BrowserStackClient.prototype, {
  connect: function connect() {
    ...
    self._launchBrowserStackTunnel(function (error) {
      ...
      self.driver = new webdriver.Builder().usingServer('http://hub.browserstack.com/wd/hub').withCapabilities(capabilities).build();
  ...
  _getBrowserStackKey: function _getBrowserStackKey() {
  ...
  _launchBrowserStackTunnel: function _launchBrowserStackTunnel(callback) {
    var self = this;
    var browserStackPath = ensureBrowserStack();

    var args = [browserStackPath, browserStackKey, [self.host, self.port, 0].join(','),
    // Disable Live Testing and Screenshots, just test with Automate.
    '-onlyAutomate',
    // Do not wait for the server to be ready to spawn the process.
    '-skipCheck'];
    self.tunnelProcess = child_process.execFile('/usr/bin/env', ['bash', '-c', args.join(' ')]);
...
function ensureBrowserStack() {
  var browserStackPath = files.pathJoin(files.getDevBundle(), 'bin', 'BrowserStackLocal');

  var browserStackStat = files.statOrNull(browserStackPath);
  if (!browserStackStat) {
    (function () {
      var host = "browserstack-binaries.s3.amazonaws.com";
      var OS = process.platform === "darwin" ? "osx" : "linux";
      var ARCH = process.arch === "x64" ? "x86_64" : "i686";
      var tarGz = 'BrowserStackLocal-07-03-14-' + OS + '-' + ARCH + '.gz';
      var url = 'https://' + host + '/' + tarGz;

      buildmessage.enterJob("downloading BrowserStack binaries", function () {
        return new Promise(function (resolve, reject) {
          var browserStackStream = files.createWriteStream(browserStackPath);
  ...
  files.chmod(browserStackPath, 493);
...
var Run = function Run(execPath, options) {
  var self = this;
...
_.extend(Run.prototype, {
  ...
  _exited: function _exited(status) {
    var self = this;
  ...
  _ensureStarted: function _ensureStarted() {
    ...
    self.proc = child_process.spawn(files.convertToOSPath(self.execPath), self._args, {
  ...
  waitSecs: function waitSecs(secs) {
    var self = this;
    self.extraTime += secs;
  },
  ...
  _stopWithoutWaiting: function _stopWithoutWaiting() {
    var self = this;
  ...
  tellMongo: markStack(function (command) {
    ...
    if (!self.fakeMongoConnection) {
      ...
      var lastStartTime = 0;
      for (var attempts = 0; !self.fakeMongoConnection && attempts < 600; attempts++) {
...
_.extend(Test.prototype, {
  onCleanup: function onCleanup(cleanupHandler) {
...
var getAllTests = function getAllTests() {
  if (allTests) {
    return allTests;
  }
  allTests = [];
...
var define = function define(name, tagsList, f) {
...
var getFilteredTests = function getFilteredTests(options) {
  options = options || {};
...
var TestList = function TestList(allTests, tagsToSkip, tagsToMatch, testState) {
...
TestList.prototype.generateSkipReport = function () {
  ...
  _.each(self.skippedTags, function (tag) {
    ...
    if (count) {
      ...
      var nounPhrase = / /.test(tag) ? noun + " " + tag : tag + " " + noun;
      // " (foo)" or ""
      var parenthetical = tagDescriptions[tag] ? " (" + tagDescriptions[tag] + ")" : '';
...
var getTestStateFilePath = function getTestStateFilePath() {
  return files.pathJoin(files.getHomeDir(), '.meteortest');
};

var readTestState = function readTestState() {
...
var runTests = function runTests(options) {
  var testList = getFilteredTests(options);
  ...
  _.each(testList.filteredTests, function (test) {
    ...
    try {
      ...
      parseStack.markBottom(function () {
        test.f(options);
      })();
    } catch (e) {
      failure = e;
    ...
    if (failure) {
      ...
      if (failure instanceof TestFailure) {
        ...
        var relpath = files.pathRelative(files.getCurrentToolsDir(), frames[0].file);
      ...
      Console.error("... ok (" + durationMs + " ms)", Console.options({ indent: 2 }));
    }
  });
