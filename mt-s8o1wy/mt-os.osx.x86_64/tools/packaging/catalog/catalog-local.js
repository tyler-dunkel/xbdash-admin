var KNOWN_ISOBUILD_FEATURE_PACKAGES;module.import('../../isobuild/compiler.js',{"KNOWN_ISOBUILD_FEATURE_PACKAGES":function(v){KNOWN_ISOBUILD_FEATURE_PACKAGES=v}});var glob;module.import("glob",{"sync":function(v){glob=v}});var Profile;module.import("../../tool-env/profile.js",{"Profile":function(v){Profile=v}});var optimisticHashOrNull;module.import("../../fs/optimistic.js",{"optimisticHashOrNull":function(v){optimisticHashOrNull=v}});var _ = require('underscore');
...
var LocalCatalog = function LocalCatalog(options) {
  var self = this;
  options = options || {};
...
_.extend(LocalCatalog.prototype, {
  ...
  initialize: function initialize(options) {
    ...
    var addPatternsToList = Profile("addPatternsToList", function (patterns, list) {
      if (!patterns) {
        return;
      }
      ...
      patterns.forEach(function (pattern) {
        ...
        glob(pattern).forEach(function (p) {
          return list.push(files.pathResolve(p));
        });
      });
    });
  ...
  _requireInitialized: function _requireInitialized() {
    var self = this;
  ...
  getAllPackageNames: function getAllPackageNames(options) {
    var self = this;
  ...
  getAllNonTestPackageNames: function getAllNonTestPackageNames() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

    var _ref$includeNonCore = _ref.includeNonCore;
    var includeNonCore = _ref$includeNonCore === undefined ? true : _ref$includeNonCore;
    ...
    var nonCoreDir = files.pathJoin(files.getCurrentToolsDir(), "packages", "non-core") + files.pathSep;

    _.each(self.packages, function (_ref2, name) {
      var sourceRoot = _ref2.packageSource.sourceRoot;
      var isTest = _ref2.versionRecord.isTest;
  ...
  getSortedVersionRecords: function getSortedVersionRecords(name) {
    ...
    if (_.has(KNOWN_ISOBUILD_FEATURE_PACKAGES, name)) {
      return KNOWN_ISOBUILD_FEATURE_PACKAGES[name].map(function (version) {
        return { version: version, packageName: name, dependencies: {} };
      });
    }
  ...
  getVersionBySourceRoot: function getVersionBySourceRoot(sourceRoot) {
    var self = this;
  ...
  // self.explicitlyAddedLocalPackageDirs.
  _computeEffectiveLocalPackages: function _computeEffectiveLocalPackages() {
    var self = this;
    buildmessage.assertInCapture();
    ...
    buildmessage.enterJob("looking for packages", function () {
      _.each(self.explicitlyAddedLocalPackageDirs, function (explicitDir) {
        var packageJsPath = files.pathJoin(explicitDir, "package.js");
        var packageJsHash = optimisticHashOrNull(packageJsPath);
      ...
      _.each(self.localPackageSearchDirs, function (searchDir) {
        ...
        _.each(possiblePackageDirs, function (subdir) {
          ...
          var packageJsPath = files.pathJoin(absPackageDir, "package.js");
          var packageJsHash = optimisticHashOrNull(packageJsPath);
  ...
  _loadLocalPackages: function _loadLocalPackages(buildingIsopackets) {
    var self = this;
    buildmessage.assertInCapture();
    ...
    var initSourceFromDir = function initSourceFromDir(packageDir, definiteName) {
    ...
    // hit node problems.
    buildmessage.forkJoin({ 'title': 'initializing packages', parallel: false }, self.effectiveLocalPackageDirs, function (dir) {
      initSourceFromDir(dir);
    });
  },


  getPackageSource: function getPackageSource(name) {
    var self = this;
    if (!_.has(self.packages, name)) return null;
