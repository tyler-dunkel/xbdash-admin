...
catalog.refreshOrWarn = function (options) {
  ...
  try {
    ...
    if (err.errorType !== 'DDP.ConnectionError') throw err;
...
catalog.runAndRetryWithRefreshIfHelpful = function (attempt) {
  ...
  var canRetry = !(catalog.triedToRefreshRecently || catalog.official.offline);
...
var ACCEPT_NON_EMPTY = function ACCEPT_NON_EMPTY(result) {
  // null, undefined
...
var LayeredCatalog = function LayeredCatalog(localCatalog, otherCatalog) {
  var self = this;
...
_.extend(LayeredCatalog.prototype, {
  ...
  _returnFirst: function _returnFirst(f, args, validityOracle) {
    var _self$localCatalog, _self$otherCatalog;

    var self = this;
    var result = (_self$localCatalog = self.localCatalog)[f].apply(_self$localCatalog, _toConsumableArray(args));
    if (validityOracle(result)) {
      return result;
    }
    return (_self$otherCatalog = self.otherCatalog)[f].apply(_self$otherCatalog, _toConsumableArray(args));
  },
  ...
  getSortedVersions: function getSortedVersions() {
  ...
  getSortedVersionRecords: function getSortedVersionRecords() {
