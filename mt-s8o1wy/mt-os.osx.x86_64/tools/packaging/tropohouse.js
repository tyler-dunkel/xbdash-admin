...
var defaultWarehouseDir = function defaultWarehouseDir() {
  // a hook for tests, or i guess for users.
  if (process.env.METEOR_WAREHOUSE_DIR) {
    return process.env.METEOR_WAREHOUSE_DIR;
  }

  var warehouseBase = files.inCheckout() ? files.getCurrentToolsDir() : files.getHomeDir();
...
exports._extractAndConvert = function (packageTarball, forceConvert) {
  ...
  if (process.platform === "win32" || forceConvert) {
    ...
    // while untarring.
    var _Isopack$readMetadata = Isopack.readMetadataFromDirectory(targetDirectory);

    var metadata = _Isopack$readMetadata.metadata;
    var originalVersion = _Isopack$readMetadata.originalVersion;
    ...
    if (originalVersion === 'unipackage-pre2' || originalVersion === 'isopack-1') {
      ...
      files.writeFile(isopackJsonPath, new Buffer(JSON.stringify(isopackFileData, null, 2), 'utf8'), { mode: 292 });
      ...
      _.each(convertedMetadata.builds, function (unibuildMeta) {
        ...
        files.writeFile(unibuildJsonPath, new Buffer(JSON.stringify(convertedUnibuild, null, 2), 'utf8'), { mode: 292 });
      ...
      _.each(convertedMetadata.plugins, function (pluginMeta) {
        ...
        files.writeFile(programJsonPath, new Buffer(JSON.stringify(convertedPlugin, null, 2), 'utf8'), { mode: 292 });
...
_.extend(exports.Tropohouse.prototype, {
  ...
  packagePath: function packagePath(packageName, version, relative) {
    var self = this;
  ...
  wipeAllPackages: function wipeAllPackages() {
    var self = this;
    ...
    if (!files.inCheckout()) {
      ...
      // eg, 'meteor-tool'
      currentToolPackageEscaped = files.pathBasename(files.pathDirname(files.pathDirname(toolsDir)));
  ...
  //    archinfo.host().
  installed: function installed(options) {
    var self = this;
  ...
  // ENOENT/
  _alreadyDownloaded: function _alreadyDownloaded(options) {
    var self = this;
    ...
    var _Isopack$readMetadata2 = Isopack.readMetadataFromDirectory(packagePath);

    var packageMetadata = _Isopack$readMetadata2.metadata;
  ...
  _saveIsopack: function _saveIsopack(isopack, packageName) {
  ...
  // buildmessage).
  _makeDownloader: function _makeDownloader(options) {
    var self = this;
    ...
    if (!buildsToDownload) {
      buildmessage.error("No compatible binary build found for this package. " + "Contact the package author and ask them to publish it " + "for your platform.", { tags: { refreshCouldHelp: true } });
      return null;
    }
    ...
    var download = function download() {
      ...
      buildmessage.enterJob({
        ...
        // warehouse?
        _.each(buildsToDownload, function (_ref) {
          var url = _ref.build.url;

          var packageTarball = buildmessage.enterJob({
            title: "downloading " + packageName + "@" + version + "..."
          }, function () {
            try {
          ...
          buildmessage.enterJob({
            ...
            var buildTempDir = exports._extractAndConvert(packageTarball);
  ...
  downloadPackagesMissingFromMap: function downloadPackagesMissingFromMap(packageMap, options) {
    var self = this;
    buildmessage.assertInCapture();
    options = options || {};
  ...
  latestMeteorSymlink: function latestMeteorSymlink() {
    var self = this;
    var linkPath = files.pathJoin(self.root, 'meteor');
    return files.readLinkToMeteorScript(linkPath, self.platform);
  },

  linkToLatestMeteor: function linkToLatestMeteor(scriptLocation) {
    var self = this;
    var linkPath = files.pathJoin(self.root, 'meteor');
