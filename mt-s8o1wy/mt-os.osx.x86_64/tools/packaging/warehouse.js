...
_.extend(warehouse, {
  ...
  getToolsDir: function getToolsDir(version) {
    return files.pathJoin(warehouse.getWarehouseDir(), 'tools', version);
  },

  getToolsFreshFile: function getToolsFreshFile(version) {
    return files.pathJoin(warehouse.getWarehouseDir(), 'tools', version, '.fresh');
  },

  _latestReleaseSymlinkPath: function _latestReleaseSymlinkPath() {
    return files.pathJoin(warehouse.getWarehouseDir(), 'releases', 'latest');
  },

  _latestToolsSymlinkPath: function _latestToolsSymlinkPath() {
  ...
  ensureReleaseExistsAndReturnManifest: function ensureReleaseExistsAndReturnManifest(release, quiet) {
    if (!files.usesWarehouse()) {
  ...
  packageExistsInWarehouse: function packageExistsInWarehouse(name, version) {
  ...
  getPackageFreshFile: function getPackageFreshFile(name, version) {
    return files.pathJoin(warehouse.getWarehouseDir(), 'packages', name, version, '.fresh');
  },

  toolsExistsInWarehouse: function toolsExistsInWarehouse(version) {
  ...
  realReleaseExistsInWarehouse: function realReleaseExistsInWarehouse(version) {
  ...
  _calculateNewPiecesForRelease: function _calculateNewPiecesForRelease(releaseManifest) {
  ...
  _packageUpdatesMessage: function _packageUpdatesMessage(packageNames) {
    var lines = [];
  ...
  _populateWarehouseForRelease: function _populateWarehouseForRelease(releaseVersion, showInstalling) {
    var releasesDir = files.pathJoin(warehouse.getWarehouseDir(), 'releases');
    files.mkdir_p(releasesDir, 493);
    var releaseManifestPath = files.pathJoin(releasesDir, releaseVersion + '.release.json');
    ...
    if (!releaseAlreadyExists) {
      ...
      try {
        var notices = httpHelpers.getUrl(WAREHOUSE_URLBASE + "/releases/" + releaseVersion + ".notices.json");
    ...
    if (newPieces && showInstalling) {
      var unlinkIfExists = function unlinkIfExists(file) {
        try {
          files.unlink(file);
        } catch (e) {
  ...
  downloadToolsToWarehouse: function downloadToolsToWarehouse(toolsVersion, platform, warehouseDirectory, dontWriteFreshFile) {
  ...
  // this function is also used by bless-release.js
  downloadPackagesToWarehouse: function downloadPackagesToWarehouse(packagesToDownload, platform, warehouseDirectory, dontWriteFreshFile) {
