module.export({bundleBuild:function(){return bundleBuild},createAndPublishBuiltPackage:function(){return createAndPublishBuiltPackage}});var _ = require('underscore');
...
var openPackageServerConnection = function openPackageServerConnection(packageServerUrl) {
...
// the file itself.
var generateBlankReadme = function generateBlankReadme() {
  return {
    contents: "",
    excerpt: "",
...
var saveReadmeToTmp = function saveReadmeToTmp(readmeInfo) {
...
exports.callPackageServer = function (conn) {
  ...
  return (_conn = conn).call.apply(_conn, args);
};
...
var loadRemotePackageData = function loadRemotePackageData(conn, syncToken, options) {
  options = options || {};
...
var _updateServerPackageData = function _updateServerPackageData(dataStore, options) {
  var self = this;
  options = options || {};
  if (dataStore === null) {
  ...
  var getSomeData = function getSomeData() {
    ...
    if (remoteData.collectionsCompressed) {
      ...
      var colsJSON = new Promise(function (resolve, reject) {
        zlib.gunzip(colsGzippedBuffer, function (err, res) {
...
_updateServerPackageData = Profile('package-client _updateServerPackageData', _updateServerPackageData);
...
var bundleSource = function bundleSource(isopack, includeSources, packageDir) {
  buildmessage.assertInJob();
  ...
  var dirToTar = files.pathJoin(tempDir, 'source', colonConverter.convert(packageTarName));
  ...
  _.each(includeSources, function (f) {
    var from = files.pathJoin(packageDir, f);
    var to = files.pathJoin(sourcePackageDir, f);
...
var uploadFile = function uploadFile(putUrl, filepath) {
  buildmessage.assertInJob();
...
var publishBuiltPackage = function publishBuiltPackage(conn, isopack, bundleResult) {
  buildmessage.assertInJob();
  var name = isopack.name;
...
exports.updatePackageMetadata = function (options) {
  ...
  buildmessage.enterJob('uploading documentation', function () {
    ...
    callPackageServerBM(conn, "publishReadme", uploadInfo.uploadToken, { hash: readmeInfo.hash });
  });
  if (buildmessage.jobHasMessages()) {
    return;
  }
  ...
  _.each(packageDeps, function (refs, label) {
    if (refs.constraint == null) {
      if (packageSource.isCore && files.inCheckout() && projectContext.localCatalog.getPackage(label)) {
        ...
        var versionString = projectContext.localCatalog.getLatestVersion(label).version;
