var main;module.import('./cli/main.js',{"default":function(v){main=v}});var buildmessage;module.import('./utils/buildmessage.js',{"default":function(v){buildmessage=v}});var cordova={};module.import('./cordova',{"*":function(v,n){cordova[n]=v}});/* eslint no-console: 0 */
...
var maybePrintNoticeHeader = function maybePrintNoticeHeader() {
...
var upgradersByName = {
  "notices-for-0.9.0": function noticesFor090(projectContext) {
  ...
  "notices-for-0.9.1": function noticesFor091() {
    maybePrintNoticeHeader();
  ...
  "0.9.4-platform-file": function platformFile(projectContext) {
    ...
    var oldPlatforms = _.compact(_.map(files.splitBufferToLines(oldPlatformsFile), files.trimSpaceAndComments));
  ...
  "notices-for-facebook-graph-api-2": function noticesForFacebookGraphApi2(projectContext) {
  ...
  "1.2.0-meteor-platform-split": function meteorPlatformSplit(projectContext) {
    var packagesFile = projectContext.projectConstraintsFile;
    ...
    if (packagesFile.getConstraint('meteor-platform')) {
      ...
      'meteor-base', 'mobile-experience', 'mongo', 'blaze-html-templates', 'session', 'jquery', 'tracker',
  ...
  "1.2.0-cordova-changes": function cordovaChanges(projectContext) {
    // Remove Cordova project directory to start afresh
    // and avoid a broken project
    files.rm_recursive(projectContext.getProjectLocalDirectory('cordova-build'));
    ...
    var pluginsFile = projectContext.cordovaPluginsFile;
    var messages = void 0;
    if (files.exists(pluginsFile.filename)) {
      messages = buildmessage.capture({ title: 'converting Cordova plugins' }, function () {
        var pluginVersions = pluginsFile.getPluginVersions();
  ...
  "1.2.0-breaking-changes": function breakingChanges() {
    maybePrintNoticeHeader();
    Console.info('Meteor 1.2 includes many changes and improvements to the build system, some of which might require small changes to apps and packages. Please read the guide about breaking changes here:', Console.url("https://github.com/meteor/meteor/wiki/Breaking-changes-in-Meteor-1.2"), Console.options({ bulletPoint: "1.2: " }));
  },

  "1.3.0-split-minifiers-package": function splitMinifiersPackage(projectContext) {
    var packagesFile = projectContext.projectConstraintsFile;
  ...
  "1.4.0-remove-old-dev-bundle-link": function removeOldDevBundleLink(projectContext) {
    var oldDevBundleLink = files.pathJoin(projectContext.projectDir, ".meteor", "dev_bundle");
    files.rm_recursive(oldDevBundleLink);
  },

  "1.4.1-add-shell-server-package": function addShellServerPackage(projectContext) {
    var packagesFile = projectContext.projectConstraintsFile;
