...
files.rm_recursive = Profile("files.rm_recursive", function (p) {
  var path = files.convertToOSPath(p);
  try {
    rimraf.sync(path);
  } catch (e) {
    if (e.code === "ENOTEMPTY" && Fiber.current && Fiber["yield"] && !Fiber["yield"].disallowed) {
...
files.cp_r = function (from, to) {
  var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

  from = files.pathResolve(from);

  var stat = statOrNull(from, options.preserveSymlinks);
  if (!stat) {
    return;
  }
  ...
  if (stat.isDirectory()) {
    ...
    files.readdir(from).forEach(function (f) {
      if (options.ignore && _.any(options.ignore, function (pattern) {
      ...
      files.cp_r(files.pathJoin(from, f), files.pathJoin(to, f), options);
    });
  ...
  if (stat.isSymbolicLink()) {
    ...
    var mode = stat.mode & 64 ? 511 : 438;

    if (options.transformContents) {
      files.writeFile(to, options.transformContents(files.readFile(from), files.pathBasename(from)), { mode: mode });
    } else {
...
files.copyFile = function (from, to) {
  var origMode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
...
files.extractTarGz = function (buffer, destPath, options) {
  ...
  var promise = process.platform === "win32" ? tryExtractWithNative7z(buffer, tempDir, options) : tryExtractWithNativeTar(buffer, tempDir, options);

  promise = promise["catch"](function (error) {
    return tryExtractWithNpmTar(buffer, tempDir, options);
  });
  ...
  .filter(function (file) {
    return !file.startsWith("PaxHeader");
  });
...
function ensureDirectoryEmpty(dir) {
  files.readdir(dir).forEach(function (file) {
...
function tryExtractWithNativeTar(buffer, tempDir, options) {
  ...
  return new Promise(function (resolve, reject) {
    var flags = options.verbose ? "-xzvf" : "-xzf";
    var tarProc = spawn("tar", [flags, "-"], {
...
function tryExtractWithNative7z(buffer, tempDir, options) {
  ...
  var exeOSPath = files.convertToOSPath(files.pathJoin(files.getCurrentNodeBinDir(), "7z.exe"));
  var tarGzBasename = "out.tar.gz";
  var spawnOptions = {
    cwd: files.convertToOSPath(tempDir),
  ...
  return new Promise(function (resolve, reject) {
    ...
    var tarBasename = void 0;
    var foundTar = files.readdir(tempDir).some(function (file) {
...
files.writeFileAtomically = function (filename, contents) {
  var parentDir = files.pathDirname(filename);
  files.mkdir_p(parentDir);

  var tmpFile = files.pathJoin(parentDir, '.' + files.pathBasename(filename) + '.' + utils.randomToken());
...
var YIELD_ALLOWED = !process.env.METEOR_DISABLE_FS_FIBERS;
...
function wrapFsFunc(fsFuncName, pathArgIndices, options) {
  ...
  function makeWrapper(_ref) {
    ...
    function wrapper() {
      ...
      var canYield = YIELD_ALLOWED && Fiber.current && Fiber["yield"] && !Fiber["yield"].disallowed;
      ...
      // these calls sync.
      var isQuickie = fsFuncName === 'stat' || fsFuncName === 'rename' || fsFuncName === 'symlink';
