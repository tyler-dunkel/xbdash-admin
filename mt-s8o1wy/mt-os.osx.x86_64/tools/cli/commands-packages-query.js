...
var getReleaseVersionPublishedOn = function getReleaseVersionPublishedOn(versionRecord) {
...
// were not shown.
var formatHiddenVersions = function formatHiddenVersions(hiddenVersions, oldestShownVersion) {
...
var formatEJSON = function formatEJSON(data) {
...
var padLongformDate = function padLongformDate(dateStr) {
...
var getTempContext = function getTempContext(options) {
  var projectContext;
...
var itemNotFound = function itemNotFound(item) {
...
// inherits from it.
var BasePkgDatum = function BasePkgDatum() {
  var self = this;
  self.data = null;
};
_.extend(BasePkgDatum.prototype, {
  // Throws if data has not been initialized.
  _checkInitialized: function _checkInitialized() {
    var self = this;
    if (self.data === null) {
  ...
  getConsoleStr: function getConsoleStr() {
    var self = this;
    self._checkInitialized();
    return "";
  }
...
var PkgExports = function PkgExports(pkgExports) {
  var self = this;
...
var PkgImplies = function PkgImplies(pkgDeps) {
  var self = this;
  self.data = [];
...
var PkgDependencies = function PkgDependencies(pkgDeps) {
  var self = this;
  self.data = _.map(
...
var PackageQuery = function PackageQuery(options) {
  var self = this;
...
_.extend(PackageQuery.prototype, {
  ...
  _getVersionRecord: function _getVersionRecord(version) {
    var self = this;
  ...
  _collectPackageData: function _collectPackageData() {
    var self = this;
    var data = {
  ...
  _getOfficialVersion: function _getOfficialVersion(versionRecord) {
    var self = this;
  ...
  //     - constraint: constraint for that dependency
  //     - weak: true if this is a weak dependency.
  _getLocalVersion: function _getLocalVersion(localRecord) {
    var self = this;
    var data = {
      name: self.name,
    ...
    data["version"] = packageSource.versionExplicitlyProvided ? localRecord.version : "local";
  ...
  _displayVersion: function _displayVersion(data) {
    var self = this;
    Console.info(data.name + "@" + data.version, Console.options({ bulletPoint: "Package: " }));
    if (data.directory) {
      Console.info("Directory: " + Console.path(data.directory));
    }
    if (data.exports && !data.exports.isEmpty()) {
      Console.info(data["exports"].getConsoleStr(), Console.options({ bulletPoint: "Exports: " }));
    }
    if (data.implies && !data.implies.isEmpty()) {
      Console.info(data["implies"].getConsoleStr(), Console.options({ bulletPoint: "Implies: " }));
    }
    if (data.git) {
      Console.info(Console.url(data.git), Console.options({ bulletPoint: "Git: " }));
    }
    ...
    if (data.dependencies && !data.dependencies.isEmpty()) {
      ...
      Console.info(data.dependencies.getConsoleStr(), Console.options({ indent: 2 }));
    }
  ...
  //   name of export.
  _generateVersionObject: function _generateVersionObject(data) {
  ...
  _displayPackage: function _displayPackage(data) {
    var self = this;
    ...
    if (data.homepage) {
      Console.info(Console.url(data.homepage), Console.options({ bulletPoint: "Homepage: " }));
    }
    // Local packages might not have any maintainers.
    if (!_.isEmpty(data.maintainers)) {
      Console.info(data.maintainers.join(", "), Console.options({ bulletPoint: "Maintainers: " }));
    }
    // Git is per-version, so we will print the latest one, if one exists.
    if (defaultVersion && defaultVersion.git) {
      Console.info(Console.url(defaultVersion.git), Console.options({ bulletPoint: "Git: " }));
    }
    // Print the exports.
    if (defaultVersion && defaultVersion.exports && !defaultVersion.exports.isEmpty()) {
      Console.info(defaultVersion["exports"].getConsoleStr(), Console.options({ bulletPoint: "Exports: " }));
    }
    if (defaultVersion && defaultVersion.implies && !defaultVersion.implies.isEmpty()) {
      Console.info(defaultVersion["implies"].getConsoleStr(), Console.options({ bulletPoint: "Implies: " }));
    }
    Console.info();
    ...
    if (data.totalVersions > versionRows.length) {
      var oldestShownVersion = data["versions"][0] && data["versions"][0].version || "";
      // A string explaining why those versions have been hidden.
      var hiddenVersions = formatHiddenVersions(data["hiddenVersions"], oldestShownVersion);
      ...
      var allVersionsPluralizer = data.totalVersions === 1 ? "the hidden version" : "all " + data.totalVersions + " versions";
  ...
  //   - exports: Array of objects representing the package exports, sorted by
  //     name of export.
  _generatePackageObject: function _generatePackageObject(data) {
    var packageFields = ["name", "homepage", "maintainers", "totalVersions"];
...
var ReleaseQuery = function ReleaseQuery(options) {
  var self = this;
...
_.extend(ReleaseQuery.prototype, {
  ...
  _getVersionDetails: function _getVersionDetails(version) {
    var self = this;
  ...
  _getReleaseData: function _getReleaseData() {
    var self = this;
    var data = {
      track: self.metaRecord.name,
      maintainers: _.pluck(self.metaRecord.maintainers, "username")
    };
    data["defaultVersion"] = catalog.official.getDefaultReleaseVersionRecord(self.name);
    ...
    // return only.
    var versionFields = ["version", "description", "recommended"];
  ...
  _displayVersion: function _displayVersion(data) {
    ...
    if (!_.isEmpty(data.packages)) {
      ...
      _.each(data.packages, function (version, packageName) {
        Console.info(packageName + ": " + version, Console.options({ indent: 2 }));
      });
      Console.info();
    }
  ...
  //         - publishedBy: username of the publisher
  //         - publishedOn: date the version was published
  _displayRelease: function _displayRelease(data) {
    var self = this;
    ...
    if (data.totalVersions > rows.length) {
      var versionsPluralizer = data.totalVersions > 1 ? "all " + data.totalVersions + " versions" : "the hidden version";
  ...
  _displayAllReleaseVersions: function _displayAllReleaseVersions(versions) {
    var self = this;
...
main.registerCommand({
  ...
  var filterBroken = function filterBroken(match, isRelease, name) {
  ...
  if (options.maintainer) {
    ...
    selector = function selector(name, isRelease) {
      var record;
      ...
      return filterBroken(name.match(search) && record && !!_.findWhere(record.maintainers, { username: username }), isRelease, name);
    };
  } else {
    selector = function selector(name, isRelease) {
