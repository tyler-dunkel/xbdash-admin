module.export({meteorCommandsJsonPath:function(){return meteorCommandsJsonPath},dumpMeteorCommands:function(){return dumpMeteorCommands}});var showRequireProfile = 'METEOR_PROFILE_REQUIRE' in process.env;
...
var meteorCommandsJsonPath = files.pathJoin(files.getDevBundle(), "bin", ".meteor-commands.json");
...
var loadHelp = function loadHelp() {
...
var longHelp = exports.longHelp = function (commandName) {
  ...
  if (!(node instanceof Command)) {
    ...
    _.each(node, function (n, shortName) {
      var fullName = commandName + (commandName.length > 0 ? " " : "") + shortName;
...
var springboard = function springboard(rel, options) {
  options = options || {};
  ...
  if (process.platform === "win32") {
    ...
    var buildsForHostArch = catalog.official.getBuildsForArches(rel.getToolsPackage(), rel.getToolsVersion(), [archinfo.host()]);

    if (!buildsForHostArch) {
      var release = catalog.official.getDefaultReleaseVersion();
      var releaseName = release.track + "@" + release.version;

      Console.error("This project uses " + rel.getDisplayName() + ", which isn't", "available on Windows. To work with this app on all supported", "platforms, use", Console.command("meteor update --release " + releaseName), "to pin this app to the newest Windows-compatible release.");
  ...
  Console.withProgressDisplayVisible(function () {
    var messages = buildmessage.capture({ title: "downloading the command-line tool" }, function () {
...
var oldSpringboard = function oldSpringboard(toolsVersion) {
  // Strip off the "node" and "meteor.js" from argv and replace it with the
  // appropriate tools's meteor shell script.
  var newArgv = process.argv.slice(2);
  var cmd = files.pathJoin(warehouse.getToolsDir(toolsVersion), 'bin', 'meteor');
...
Fiber(function () {
  ...
  var walkCommands = function walkCommands(node) {
  ...
  if (process.getuid) {
    ...
    if (process.getuid() === 0) {
      if (_.has(rawOptions, '--unsafe-perm')) {
        ...
        Console.info("");
        Console.info("You have run Meteor as root.", "Your permissions in your app directory will be incorrect if you ever attempt to perform any Meteor tasks as your non-root user.", "You probably didn't want this, but you can fix it by running the following from the root of your project:");
        Console.info("");
  ...
  if (releaseName !== undefined) {
    ...
    if (!rel) {
      ...
      var displayRelease = catalogUtils.displayRelease(trackAndVersion[0], trackAndVersion[1]);
  ...
  if (release.current && release.current.isProperRelease() && release.current.getToolsPackageAtVersion() !== files.getToolsVersion()) {
  ...
  _.each(command.options, function (optionInfo, optionName) {
    ...
    var helpfulOptionName = "--" + optionName + (presentShort ? " (-" + optionInfo.short + ")" : "");
  ...
  if (requiresApp && !options.appDir) {
    ...
    Console.error(Console.command("meteor create <project name>"), Console.options({ indent: 2 }));
    Console.error("For example:");
    Console.error(Console.command("meteor create myapp"), Console.options({ indent: 2 }));
    Console.error();
  ...
  var requiresAppOrPackage = command.evaluateOption('requiresAppOrPackage', options);
  ...
  try {
    ...
    var ret = command.func(options, { rawOptions: rawOptions });
  } catch (e) {
