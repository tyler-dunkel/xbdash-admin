var _slicedToArray;module.import('babel-runtime/helpers/slicedToArray',{"default":function(v){_slicedToArray=v}});var cordova={};module.import('../cordova',{"*":function(v,n){cordova[n]=v}});var updateMeteorToolSymlink;module.import("../packaging/updater.js",{"updateMeteorToolSymlink":function(v){updateMeteorToolSymlink=v}});
...
var getReleaseOrPackageRecord = function getReleaseOrPackageRecord(name) {
...
var refreshOfficialCatalogOrDie = function refreshOfficialCatalogOrDie(options) {
...
var removeIfEndsWith = function removeIfEndsWith(s, suffix) {
...
main.registerCommand({
  ...
  var addPackages = function addPackages(packageNames) {
...
var updatePackageMetadata = function updatePackageMetadata(packageSource, conn) {
  ...
  Console.info("Success. You can take a look at the new metadata by running", Console.command("'meteor show " + name + "@" + version + "'"), "outside the current project directory.");
...
main.registerCommand({
  ...
  if (options.create) {
    ...
    if (!options['top-level'] && !packageName.match(/:/)) {
      Console.error("Only administrators can create top-level packages without an", "account prefix. (To confirm that you wish to create a top-level", "package with no account prefix, please run this command again", "with the --top-level option.)");
  ...
  if (!options['no-lint']) {
    var warnings = projectContext.getLintingMessagesForLocalPackages();
  ...
  if (binary && options['existing-version']) {
    ...
    _.each(["os.osx.x86_64", "os.linux.x86_64", "os.linux.x86_32", "os.windows.x86_32"], function (a) {
      Console.info(Console.command("meteor admin get-machine " + a), Console.options({ indent: 2 }));
    });

    Console.info();
    Console.info("On each machine, run:");
    Console.info();
    Console.info(Console.command("meteor publish-for-arch " + packageSource.name + "@" + packageSource.version), Console.options({ indent: 2 }));
    Console.info();
    Console.info("For more information on binary ABIs and consistent builds, see:");
    Console.info(Console.url("https://github.com/meteor/meteor/wiki/Build-Machines"), Console.options({ indent: 2 }));
    Console.info();
  }
...
main.registerCommand({
  ...
  buildmessage.enterJob("extracting package source", function () {
  ...
  files.copyFile(files.pathJoin(packageDir, '.versions'), files.pathJoin(tempProjectDir, '.meteor', 'versions'));
    ...
    main.captureAndExit("=> Errors in release packages:", function () {
      _.each(allPackages, function (packageName) {
        buildmessage.enterJob("checking consistency of " + packageName, function () {
          ...
          var oldVersionRecord = catalog.official.getVersion(packageName, packageSource.version);
  ...
  main.captureAndExit("=> Errors while publishing release:", "publishing release", function () {
    ...
    if (options['create-track']) {
      ...
      buildmessage.enterJob("creating a new release track", function () {
        packageClient.callPackageServerBM(conn, 'createReleaseTrack', { name: relConf.track });
      });
      if (buildmessage.jobHasMessages()) {
        return;
      }
...
main.registerCommand({
  ...
  if (newVersionsAvailable) {
    Console.info();
    Console.info("New versions of these packages are available! Run", Console.command("'meteor update'"), "to try to update those", "packages to their latest versions. If your packages cannot be", "updated further, try typing", Console.command("`meteor add <package>@<newVersion>`"), "to see more information.", Console.options({ bulletPoint: "* " }));
  }
  if (anyBuiltLocally) {
    Console.info();
    Console.info("These packages are built locally from source.", Console.options({ bulletPoint: "+ " }));
  }
  return 0;
});

var getNewerVersion = function getNewerVersion(packageName, curVersion, whichCatalog) {
...
var maybeUpdateRelease = function maybeUpdateRelease(options) {
  ...
  var releaseTrack = release.current ? release.current.getReleaseTrack() : catalog.DEFAULT_TRACK;
  ...
  if (!options.appDir) {
    if (release.forced) {
      ...
      // help --release xyz'.
      Console.info("Installed. Run", Console.command("'meteor update --release " + release.current.getDisplayName({ noPrefix: true }) + "'"), "inside of a particular project directory to update that project to", release.current.getDisplayName() + ".");
    } else {
  ...
  if (options.patch) {
    ...
    var patchRecord = catalog.official.getReleaseVersion(projectContext.releaseFile.releaseTrack, updateTo);
  ...
  Console.info(files.pathBasename(options.appDir) + ": updated to " + projectContext.releaseFile.displayReleaseName + ".");
...
main.registerCommand({
  ...
  if (options.args.length === 0) {
    ...
    if (options["all-packages"]) {
      upgradePackageNames = _.filter(_.keys(projectContext.packageMapFile.getCachedVersions()), function (packageName) {
        return !compiler.isIsobuildFeaturePackage(packageName);
      });
    }
  ...
  var upgradePackagesWithoutCordova = upgradePackageNames.filter(function (name) {
  ...
  if (!_.isEqual(upgradePackagesWithoutCordova, upgradePackageNames)) {
    ...
    Console.warn();
    Console.warn(Console.command("meteor add cordova:PLUGIN-NAME@x.y.z"), Console.options({ indent: 2 }));
    Console.warn();
    ...
    Console.warn();
    Console.warn(Console.command("meteor npm view PLUGIN-NAME version"), Console.options({ indent: 2 }));
  ...
  if (!options.args.length) {
    ...
    var printItem = function printItem(rec) {
    ...
    if (nonlatestIndirectDeps.length) {
      ...
      Console.info(["To update one or more of these packages, pass their names to ", "`meteor update`, or just run `meteor update --all-packages`."].join("\n"));
    }
  }
});
...
main.registerCommand({
  ...
  var _cordova$splitPlugins = cordova.splitPluginsAndPackages(options.args);

  var pluginsToAdd = _cordova$splitPlugins.plugins;
  var packagesToAdd = _cordova$splitPlugins.packages;
  ...
  if (!_.isEmpty(pluginsToAdd)) {
    ...
    for (var _iterator = pluginsToAdd, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
      if (_isArray) {
      ...
      var _id = _target$split2[0];
      var version = _target$split2[1];


      var newId = cordova.newPluginId(_id);

      if (!(version && utils.isValidVersion(version, { forCordova: true }))) {
        Console.error(_id + ': Meteor requires either an exact version (e.g. ' + _id + '@1.0.0), a Git URL with a SHA reference, or a local path.');
        exitCode = 1;
      } else if (newId) {
        plugins[newId] = version;
        Console.info('Added Cordova plugin ' + newId + '@' + version + ' (plugin has been renamed as part of moving to npm).');
        changed = true;
      } else {
        plugins[_id] = version;
        Console.info('Added Cordova plugin ' + _id + '@' + version + '.');
        changed = true;
      }
    }
  ...
  var messages = buildmessage.capture(function () {
    _.each(packagesToAdd, function (packageReq) {
      buildmessage.enterJob("adding package " + packageReq, function () {
        ...
        var current = projectContext.projectConstraintsFile.getConstraint(constraint['package']);
...
main.registerCommand({
  ...
  var _cordova$splitPlugins2 = cordova.splitPluginsAndPackages(options.args);

  var pluginsToRemove = _cordova$splitPlugins2.plugins;
  var packages = _cordova$splitPlugins2.packages;
  ...
  if (!_.isEmpty(pluginsToRemove)) {
    ...
    for (var _iterator2 = pluginsToRemove, _isArray2 = Array.isArray(_iterator2), _i2 = 0, _iterator2 = _isArray2 ? _iterator2 : _iterator2[Symbol.iterator]();;) {
      if (_isArray2) {
      ...
      var newId = cordova.newPluginId(id);
      ...
      if (/@/.test(id)) {
        ...
        Console.info('Removed Cordova plugin ' + newId + ' (plugin has been renamed as part of moving to npm).');
        changed = true;
      } else {
...
main.registerCommand({
  ...
  Console.error('Building bootstrap tarballs for architectures ' + osArches.join(', '));
  ...
  _.each(osArches, function (osArch) {
    ...
    var tmpTropo = new tropohouse.Tropohouse(files.pathJoin(tmpdir, '.meteor'), { platform: targetPlatform });
