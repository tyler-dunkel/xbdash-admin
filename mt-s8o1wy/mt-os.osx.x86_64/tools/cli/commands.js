module.export({parseServerOptionsForRunCommand:function(){return parseServerOptionsForRunCommand},parseRunTargets:function(){return parseRunTargets}});var cordova={};module.import('../cordova',{"*":function(v,n){cordova[n]=v}});var CordovaProject;module.import('../cordova/project.js',{"CordovaProject":function(v){CordovaProject=v}});var CordovaRunner;module.import('../cordova/runner.js',{"CordovaRunner":function(v){CordovaRunner=v}});var iOSRunTarget,AndroidRunTarget;module.import('../cordova/run-targets.js',{"iOSRunTarget":function(v){iOSRunTarget=v},"AndroidRunTarget":function(v){AndroidRunTarget=v}});var EXAMPLE_REPOSITORIES;module.import('./example-repositories.js',{"EXAMPLE_REPOSITORIES":function(v){EXAMPLE_REPOSITORIES=v}});var main = require('./main.js');
...
var qualifySitename = function qualifySitename(site) {
  if (site.indexOf(".") === -1) {
...
var showInvalidArchMsg = function showInvalidArchMsg(arch) {
...
function parseServerOptionsForRunCommand(options, runTargets) {
  ...
  var parsedMobileServerUrl = void 0;
...
function parseMobileServerOption(mobileServerOption) {
  var optionName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'mobile-server';
...
function doRunCommand(options) {
  ...
  var _parseServerOptionsFo = parseServerOptionsForRunCommand(options, runTargets);

  var parsedServerUrl = _parseServerOptionsFo.parsedServerUrl;
  var parsedMobileServerUrl = _parseServerOptionsFo.parsedMobileServerUrl;
...
main.registerCommand({
  ...
  if (options['package']) {
    ...
    try {
      files.cp_r(files.pathJoin(__dirnameConverted, '..', 'static-assets', 'skel-pack'), packageDir, {
        transformFilename: function transformFilename(f) {
          return transform(f);
        },
        transformContents: function transformContents(contents, f) {
  ...
  if (options.list) {
    ...
    _.each(EXAMPLE_REPOSITORIES, function (repoInfo, name) {
      var branchInfo = repoInfo.branch ? '#' + repoInfo.branch : '';
      Console.info(Console.command(name + ': ' + repoInfo.repo + branchInfo), Console.options({ indent: 2 }));
    });
  ...
  if (options.example) {
    var repoInfo = EXAMPLE_REPOSITORIES[options.example];
    if (!repoInfo) {
    ...
    var branchOption = repoInfo.branch ? ' -b ' + repoInfo.branch : '';
    var path = options.args.length === 1 ? ' ' + options.args[0] : '';

    Console.info('To create the ' + options.example + ' example, please run:');
    Console.info(Console.command('git clone ' + repoInfo.repo + branchOption + path), Console.options({ indent: 2 }));
...
var buildCommand = function buildCommand(options) {
  Console.setVerbose(!!options.verbose);
  if (options.headless) {
  ...
  var serverOnly = options._bundleOnly || !!options['server-only'];
    ...
    (function () {
      ...
      main.captureAndExit('', function () {
        buildmessage.enterJob({ title: "preparing Cordova project" }, function () {
          ...
          var pluginVersions = cordova.pluginVersionsFromStarManifest(bundleResult.starManifest);
        ...
        for (var _iterator = cordovaPlatforms, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
          ...
          buildmessage.enterJob({ title: 'building Cordova app for ' + cordova.displayNameForPlatform(platform) }, function () {
            var buildOptions = { release: !options.debug };
...
main.registerCommand({
  ...
  requiresApp: function requiresApp(options) {
...
main.registerCommand({
  ...
  var rawOptions = _ref.rawOptions;
...
main.registerCommand({
  ...
  pretty: function pretty(options) {
...
main.registerCommand(_.extend({ name: 'test-packages' }, testCommandOptions), function (options) {
...
function doTestCommand(options) {
  ...
  var _parseServerOptionsFo2 = parseServerOptionsForRunCommand(options, runTargets);

  var parsedServerUrl = _parseServerOptionsFo2.parsedServerUrl;
  var parsedMobileServerUrl = _parseServerOptionsFo2.parsedMobileServerUrl;
  ...
  if (options["test-packages"]) {
    ...
    var copyDirIntoTestRunnerApp = function copyDirIntoTestRunnerApp(allowSymlink) {
      ...
      var appDirPath = files.pathJoin.apply(files, [options.appDir].concat(parts));
      var testDirPath = files.pathJoin.apply(files, [testRunnerAppDir].concat(parts));
...
var getTestPackageNames = function getTestPackageNames(projectContext, packageNames) {
...
var runTestAppForPackages = function runTestAppForPackages(projectContext, options) {
  var buildOptions = {
    minifyMode: options.production ? 'production' : 'development'
  };
...
var loggedInAccountsConnectionOrPrompt = function loggedInAccountsConnectionOrPrompt(action) {
...
main.registerCommand({
  ...
  var compileRegexp = function compileRegexp(str) {
    try {
...
main.registerCommand({
  ...
  var maybeLog = function maybeLog(string) {
    if (options.verbose) {
      Console.info(string);
    }
  };
